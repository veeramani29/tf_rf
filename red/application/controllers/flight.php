...
class Flight extends CI_Controller {
        ...
        if (isset($get['journey_type'])) {
            if ($get['journey_type'] == 'one_way') {
               ...
               // echo "<pre>";print_r($data);
                $this->load->view('flight_new/search', $data);
	...
	public function searchMultiCity($data) 
	{
		echo "<pre>";
		print_r($data);
		exit;
...
if(isset($data['route'])){
    ...
    public function getSearchData($apiId) {
        ...
        if ($data['searchParms']['journey_type'] == 'round_trip') {
            ...
            if (trim($data['searchParms']['fromCountry']) == 'Philippines' && trim($data['searchParms']['toCountry']) == 'Philippines') {
                $serchView = $this->load->view('flight_new/roundtrip_dom_results', $data, true);
                $flag = 'round_filter';
            } else {
                $serchView = $this->load->view('flight_new/roundtrip_results', $data, true);
            }
        } else {									
            $data['flights'] = $this->Viabeta_Model->getFlightAvailabilityOw($data['searchParms']);			
            if(isset($data['flights']['via_data'])){
            ...
            $serchView = $this->load->view('flight_new/oneway_results', $data, true);
        }
    ...
    public function details($id = '', $outId = '') {
		...
		//echo "<pre>";print_r($data['flightData']);die;
             $this->load->view('flight_new/flight-details', $data);
           ...
           if ($flightDetails['booking_final_data'] != '') {
            ...
            $this->load->view('flight_new/flight-details-round', $data);
        }
    }

     public function fareRulesRetrive() {
         $fare_key=($_POST['fare_key']);
      
        $fareRulesDetails = $this->Viabeta_Model->fareRulesRetrive($fare_key);
      
        echo json_encode($fareRulesDetails,true);
    }
