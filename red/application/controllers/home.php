...
class Home extends CI_Controller {
        ...
        if (isset($_SESSION['agent']) && $_SESSION['agent']['logged_in'] === '1') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data['agent_pass'] = $this->input->post('agent_pass');
                 $this->load->view('home_new/login', $data);
    ...
    public function register($status = '') {
        if (isset($_SESSION['agent']) && $_SESSION['agent']['logged_in'] === '1') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view('home_new/register', $data);
            } else {
                $data['agent_name'] = $this->input->post('agent_name');
                $data['agent_email'] = $this->input->post('agent_email');
                $data['company_name'] = $this->input->post('company_name');
    ...
    public function dashboard() {
        if (isset($_SESSION['agent']) && $_SESSION['agent']['logged_in'] === '1') {
			...
			if($_FILES)
			{
				if (isset($_FILES['user_logo']['name'])) 
				{ 
                    $filename = $_FILES['user_logo']['name'];
                    $tmpFileName = $_FILES['user_logo']['tmp_name'];
					...
					$updateUserLogo =  $this->Home_Model->updateUserLogo($user_id,$newFileName);
					$data["Megs"] = 'Your file uploaded successfully !!!';   
					
                } 
			}
			if(isset($_POST) && !empty($_POST))
			{
				if($_POST['add_staff'] == "Add New Staff")
				{
				
					$data['agent_name'] = $this->input->post('username');
					$data['user_password'] = $this->input->post('staff_conf_pass');
					$data['staff_type'] = $this->input->post('stafftype');
					$data['user_email'] = $this->input->post('staff_email');
					$register = $this->Home_Model->registerNewStaffData($data);
					if ($register !== '') {
						$data['agent_email'] = $data['user_email'];
						$data['pass'] = $register['password'];
						$data['user_no'] = $register['user_no'];
						##################### MAIL TO USER  ##################################
						//$to = trim($data['agent_email']);
						$to = "srinivasanpri21@gmail.com";
						$subject = 'Welcome to Redtag Travels - Agent Portal';
						$headers = "From: info@redtagtravels.com\r\n";
						$headers .= "Reply-To: info@redtagtravels.com\r\n";
						$headers .= "CC: pm.ajtravellabs@gmail.com\r\n";
						...
						$data["Megs"] = "New staff user added successfully !!!";
						
					} 
				}
				else if($_POST['staff_change_password'] == "Staff change password")
				{
					$data['user_id'] = $this->input->post('staff_change_pass');
					$data['user_password'] = $this->input->post('confirm_password_staff');
					$register = $this->Home_Model->updateStaffPassword($data);
					$data["Megs"] = "Staff password updated successfully !!!";
				}
				else
				{
					$user_id = $_SESSION['agent']['user_id'];
					...
					$updateUser =  $this->Home_Model->updateUserDetailsFromControlPanel($user_id,$address,$phone_number,$state,$city,$pin_code);
					
					$data["Megs"] = "Your details were updated successfully !!!";
					$this->session->set_flashdata('message', 'value 123!!!');
				}
			...
			$data['staff_data'] = $this->Home_Model->getAgentStaffDetails($_SESSION['agent']['user_id']);
			$data['state_list'] = $this->Home_Model->getAllStatesList();
			$data['country_list'] = $this->Home_Model->getAllCountryList();
            $this->load->view('home/cpanal_dashboard', $data);
        } else {
            redirect('home/login');
        }
    }
	...
	public function getCitiesList($cityiD = "")
	{
		 $stateId = $this->input->post('id');
		 $data['city_list'] = $this->Home_Model->getAllCitiesByState($stateId);
		 if($cityiD)
		 {
			$data['city_id'] = $cityiD; 
		 }
		 $this->load->view('home/citiesList', $data);
	}
	
	public function checkExitsPassword()
	{
		$oldPass = $this->input->post('username');
		$passwordChecking = $this->Home_Model->checkExitsPassword($oldPass);
		
		 
		 echo json_encode(array('result' => $passwordChecking));
	}
        ...
        if (isset($_POST)) {
            ...
            if ($data['old_pass'] == '') {
                ...
                if ($updatePass != '' || $updatePass > 0) {
					...
					//$toUser = "srinivasanpri21@gmail.com";
    ...
    public function searchengine() {
        if (isset($_SESSION['agent']) && $_SESSION['agent']['logged_in'] === '1') {
            ...
            $this->load->view('home_new/searchengine',$data);
        } else {
            redirect('home/dashboard');
        }
    }
