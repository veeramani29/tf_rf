...
$reqfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/'.$searchParms['journey_type']."req.json", "w") or die("Unable to open file!");
$rspfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/'.$searchParms['journey_type']."rsp.json", "w") or die("Unable to open file!");
...
curl_close($curlConnection);
    ...
    function flightDetails($onward, $return = '', $requestId = '', $searchData, $admin_markup, $agent_markup,$flgtType) {
	   ...
	   if ($searchData['journey_type'] == 'one_way') {
        ...
        $reqfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/'.$searchData['journey_type']."PricecheckReq.json", "w") or die("Unable to open file!");
$rspfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/'.$searchData['journey_type']."PricecheckRsp.json", "w") or die("Unable to open file!");
fwrite($reqfile, $postData);
fwrite($rspfile, $results);
fclose($reqfile);fclose($rspfile);
       ...
       curl_setopt($curlConnection, CURLOPT_POSTFIELDS, $BookpostData_);
        ...
        $results = curl_exec($curlConnection);
         $res = json_decode($results, true);  
           ...
           $reqfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/BookReq.json', "w") or die("Unable to open file!");
$rspfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/BookRsp.json', "w") or die("Unable to open file!");
...
fclose($reqfile);fclose($rspfile);
       ...
       $totalFare =$priceFinalData['fare']['totalFare']['total']['amount'];
        ...
        $path = $_SERVER['DOCUMENT_ROOT'] . '/agent/assets/xml_logs/' . $masterId . '_' . date('Y-m-d H:i:s') . '_booking_prepare.txt';
...
$rspfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/RetrieveRsp.json', "w") or die("Unable to open file!");
fwrite($rspfile, $results);
fclose($rspfile);
     ...
     function fareRulesRetrive($Key){

       
        $curlConnection = curl_init();
        curl_setopt($curlConnection, CURLOPT_HTTPHEADER, array( "Content-Encoding: UTF-8","Content-Type: application/json","Via-Access-Token: ".$this->access_Token));
        curl_setopt($curlConnection, CURLOPT_URL, $this->new_url."fare-rule/".$Key);
        curl_setopt($curlConnection, CURLOPT_FOLLOWLOCATION, TRUE);
        curl_setopt($curlConnection, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curlConnection, CURLOPT_SSL_VERIFYPEER, FALSE);
        $results = curl_exec($curlConnection);
...
$rspfile = fopen($_SERVER['DOCUMENT_ROOT'].'/agent/xml_logs/fareRules.json', "w") or die("Unable to open file!");
fwrite($rspfile, $results);
fclose($rspfile);

        $retrieveData=array('fareRules' => (isset($res['fareRules'])?$res['fareRules']:''));
        return $retrieveData;
