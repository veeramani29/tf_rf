...
class Home extends CI_Controller {
    ...
    public function flight_autofill() {
        ...
        foreach ($result as $val) {
            array_push($data, trim($val['airport_name']) . ' , ' . trim($val['airport_country']) . ' , ' . trim($val['airport_code']));
        }
...
//
//        foreach($result['hotelspro'] as $val) {
//                array_push($data, trim($val['City']).' , ' .trim($val['Country']).($val["State"]!=''?" , ".$val["State"]:''));	
//        }
    ...
    public function searchengine() {
        if (isset($_SESSION['agent']) && $_SESSION['agent']['logged_in'] === '1') {
            $this->load->view('home/searchengine');
        } else {
            redirect('home/dashboard');
        }
    }
