...
<div class="rs_flight_details" style="display: block;">
    <div class="rs_flight_details__header">
        ...
        <span class="rs_text_bold">Departing Flight : <?php echo date('D, M d, Y', strtotime($indepDetails[0])); ?></span> (Arrive - <?php echo date('D, M d, Y', strtotime($inarvDetails[0])); ?>)
    ...
    foreach ($flight['Inbound']['legs'] as $stops) {
        ...
        <div class="rs_flight_details__leg">
            ...
            <div class="rs_flight_details__leg__left"><img src="<?php echo base_url(); ?>assets/images/airline_logo/<?php echo $stops['Carrier_code']; ?>.gif"></div>
        ...
        if (count($flight['Inbound']['legs']) > 1 && $j < count($flight['Inbound']['legs']) - 1) {
            ###############################Duration Calculation##################################################
            $date3 = new DateTime($arvDateTim[0] . 'T' . $result1);
...
<div class="rs_flight_details" style="display: block;">
    <div class="rs_flight_details__header">
        ...
        <span class="rs_text_bold">Departing Flight : <?php echo date('D, M d, Y', strtotime($outdepDetails[0])); ?></span> (Arrive - <?php echo date('D, M d, Y', strtotime($outarvDetails[0])); ?>)
        <div class="rs_flight_details__highlightBar rs_highlight--red">
    ...
    foreach ($flight['Outbound']['legs'] as $stops) {
        ...
        $result1 = implode(':', $chunks1);
        $arvTime = explode(':', $result1);
        $time_in_12_hour_format_dep = date("g:i a", strtotime($result1));
        $time_in_12_hour_format_arv = date("g:i a", strtotime($result1));
