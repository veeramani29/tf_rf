...
class Via_Model extends CI_Model {
    ...
    function getFlightAvailabilityOw($searchParms) {
        ...
        $xml = '';
        if($searchParms['pref_airline'] != ''){
            if (strpos($searchParms['pref_airline'], ',') !== false){
                $carIdEx = explode(',',$searchParms['pref_airline']);
                $carrierId = trim($carIdEx[1]);
            } else {
                $carrierId = '';
            }
        } else {
        ...
        if($searchParms['sr_ctzn'] == 'true'){
            $srCitizen = '<SeniorCitizens>'.trim($searchParms['adult']).'</SeniorCitizens>';
        } else {
            $srCitizen = '';
        }
        ...
        $xml .= '<AirFareSearchRequest>
            ...
            if($srCitizen != ''){
                ...
                $xml .= '<Adults>' . trim($searchParms['adult']) . '</Adults>
                        <Children>' . trim($searchParms['child']) . '</Children>
                        <Infants>' . trim($searchParms['infant']) . '</Infants>';
            }
            $xml .= '</PassengerCount>
                     <CabinPreference>
                        <SeatingClass>'.$searchParms['travelClass'].'</SeatingClass>
                     </CabinPreference>
                     '.$carrier.'
                ...
                if ($res->getElementsByTagName('OnwardPricedItinerary')->length > 0) {
                    ...
                    foreach ($onwardFlights as $flights) {
                        ...
                        foreach ($charges as $charge) {
                            ...
                            if ($charge->getAttribute('type') == 'SingleSeniorCitizen' && $charge->getAttribute('ChargeType') == 'TotalAmount')
                                $viaData[$i]['Inbound']['adult_fare'] = $charge->nodeValue;
    ...
    function getFlightAvailabilityRt($searchParams) {
        $request_id = 'RDT' . date('YmdHis');
        $xml = '';
        if($searchParams['pref_airline'] != ''){
            if (strpos($searchParams['pref_airline'], ',') !== false){
                $carIdEx = explode(',',$searchParams['pref_airline']);
                $carrierId = trim($carIdEx[1]);
            } else {
                $carrierId = '';
            }
        ...
        if($searchParams['sr_ctzn'] == 'true'){
            $srCitizen = '<SeniorCitizens>'.trim($searchParams['adult']).'</SeniorCitizens>';
        } else {
            $srCitizen = '';
        }
        $xml .= '<AirFareSearchRequest>
                    <RequestId>' . $request_id . '</RequestId>
                    <Route>
                      <Source>' . trim($searchParams['fromCode']) . '</Source>
                      <Destination>' . trim($searchParams['toCode']) . '</Destination>
                      <DateOfDeparture>' . trim($searchParams['sd']) . '</DateOfDeparture>
                      <DateOfArrival>' . trim($searchParams['ed']) . '</DateOfArrival>
                    </Route>
                    <PassengerCount>';
            if($srCitizen != ''){
                $xml .= $srCitizen;
            } else {
                $xml .= '<Adults>' . trim($searchParams['adult']) . '</Adults>
                        <Children>' . trim($searchParams['child']) . '</Children>
                        <Infants>' . trim($searchParams['infant']) . '</Infants>';
            }
            $xml .= '</PassengerCount>
                     <CabinPreference>
                        <SeatingClass>'.$searchParams['travelClass'].'</SeatingClass>
                     </CabinPreference>
                     '.$carrier.'
                </AirFareSearchRequest>';
//echo $xml;die;
       ...
       //echo $response;die; 
        ...
        if ($response != '') {
            $res = new DOMDocument();
            $res->loadXML($response);
            if (trim($searchParams['fromCountry']) == 'Philippines' && trim($searchParams['toCountry']) == 'Philippines') {
                ...
                if ($res->getElementsByTagName('ReturnPricedItinerary')->length > 0) {
                    ...
                    foreach ($returnFlights as $flights) {
                        ...
                        foreach ($charges as $charge) {
                            ...
                            if ($charge->getAttribute('type') == 'SingleSeniorCitizen' && $charge->getAttribute('ChargeType') == 'TotalAmount')
                                $viaData[$i]['Outbound']['adult_fare'] = $charge->nodeValue;
                        ...
                        foreach ($legs as $leg) {
                            ...
                            $filters['Rairlines'][] = $viaData['Outbound'][$i]['legs'][$j]['Carrier'];
                            $j++;
                        }
                        $i++;
                        $count = $count + 1;
    ...
    function flightDetails($onward, $return = '', $requestId = '', $searchData, $admin_markup, $agent_markup,$flgtType) {
        ...
        $xml .= '<AirPriceRequest>
                    ...
                    <PassengerCount>';
                if($searchData['sr_ctzn'] == 'true'){
                    $xml .= '<SeniorCitizens>'.$searchData['adult'].'</SeniorCitizens>';
                } else {
                    $xml .= '<Adults>' . $searchData['adult'] . '</Adults>
                    ...
                    if ($res->getElementsByTagName('PricedItinerary')->length > 0) {
                        ...
                        foreach ($charges as $charge) {
                            ...
                            if ($k == 0 && $booking_final_data['fare'][$k]['ChargeType'] != 'totalAmount') {
                                $booking_final_data['fare'][$k]['price'] = ($charge->nodeValue+$mark+$agentmark);
                            ...
                            if($charge->getAttribute('type') == 'SingleAdult' && $charge->getAttribute('ChargeType') == 'BaseFare'){
                                $booking_final_data['SingleAdultBaseFare'] = ($charge->nodeValue+$mark+$agentmark);
                            }
                            if($charge->getAttribute('type') == 'SingleChild' && $charge->getAttribute('ChargeType') == 'BaseFare'){
                                $booking_final_data['SingleChildBaseFare'] = ($charge->nodeValue+$mark+$agentmark);
                            }
                            if($charge->getAttribute('type') == 'SingleInfant' && $charge->getAttribute('ChargeType') == 'BaseFare'){
                                $booking_final_data['SingleInfantBaseFare'] = ($charge->nodeValue+$mark+$agentmark);
                            }
                            if($charge->getAttribute('type') == 'SingleSeniorCitizen' && $charge->getAttribute('ChargeType') == 'BaseFare'){
                                $booking_final_data['SingleAdultBaseFare'] = ($charge->nodeValue+$mark+$agentmark);
                            }
                            $k++;
                        }
                ...
                if ($res->getElementsByTagName('AirPriceResponse')->length > 0) {
                    ...
                    if ($res->getElementsByTagName('PricedItinerary')->length > 0) {
                        ...
                        if($flgtType == 'dom'){
                            if (array_key_exists($flCode, $admin_markup)) {
                                if ($admin_markup['type'] == 'fixed') {
                                    $markOn = ($admin_markup[$flCode]);
                                    $markRt = ($admin_markup[$flCodeR]);
                                    $mark = ($markOn+$markRt);
                                    $tot = ($tot + $mark);
                                } else {
                                    $markOn = (($admin_markup[$flCode] / 100) * $tot);
                                    $markRt = (($admin_markup[$flCodeR] / 100) * $tot);
                                    $mark = ($markOn+$markRt);
                                    $tot = ($tot + $mark);
                                }
                            } else {
                        ...
                        if($flgtType == 'dom'){
                            if (array_key_exists($flCode, $agent_markup)) {
                                if ($agent_markup['type'] == 'fixed') {
                                    ...
                                    $markOnA = (($agent_markup[$flCode] / 100) * $tot);
                                    $markRtA = (($agent_markup[$flCodeR] / 100) * $tot);
                                    $agentmark = ($markOnA+$markRtA);
                                    $tot = ($tot + $agentmark);
                                }
                            } else {
