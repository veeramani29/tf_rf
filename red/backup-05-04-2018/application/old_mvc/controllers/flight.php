...
class Flight extends CI_Controller {
    ...
    public function searchOneWay($data){
        if($data['from'] != '' && $data['to'] != '' && $data['sd'] != ''){
            ...
            $searchArr = array(
                            ...
                            'infant'=>$infant,'travelClass'=>$travelClass,'nonStopOnly'=>$nonStopOnly,
        ...
        if($data['flights']['via_data'] != ''){
            if ($data['flights']['filters']['airlines'] != '') {
                ...
                foreach ($airlineList as $name) {
                    $airlines .= '<li style="width: 100%;padding: 7px 0 5px 15px;" ><label style="font-weight:normal;width: 84%" onclick="filter();"><span style="margin-top:3px;"><input type="checkbox" name="airlines" class="airlines" value="' . $name . '" style="width:15px;height:15px;margin-right:10px;" checked="checked"></span>' . $name . '</label></li>';
                }
            }
        }
        echo json_encode(
                array(
                    ...
                    'airlines' => $airlines,
                    'flight_count' => $data['flights']['filters']['count']
                // 'stops'=>''
                )
        );
    }
    ...
    public function details($id = '', $outId = '') {
        ...
        if ($data['searchData']['journey_type'] == 'one_way') {
            ...
            $flightDetails = $this->Via_Model->flightDetails($data['flightData'], $returnId = '', $requestId,$data['searchData'],$admin_markup);
            ...
            if (trim($data['searchData']['fromCountry']) == 'Philippines' && trim($data['searchData']['toCountry']) == 'Philippines') {
                ...
                $flightDetails = $this->Via_Model->flightDetails($data['flightData'], $data['roundData'], $requestId,$data['searchData'],$admin_markup);
                $data['flightDetails'] = $flightDetails['booking_final_data'];
            } else {
                //echo 'hellll';die;
                ...
                $flightDetails = $this->Via_Model->flightDetailsIntl($onwardId, $returnId = '', $requestId, $data['searchData'],$admin_markup);
                $data['flightDetails'] = $flightDetails['booking_final_data'];
            }
        ...
        if ($flightData != '') {
            ...
            if(array_key_exists($flight['legs'][0]['Carrier_code'],$admin_markup)){
                ...
                if($admin_markup['type'] == 'fixed'){
                    $admMark = ($admin_markup[$airCode]/2);
                    $totalFare = ($totalFare+$admMark);
                } else {
                    $admMark = ((($admin_markup[$airCode]/100)*$totalFare)/2);
            ...
            if ($outId != '') {
                ...
                if ($infant > 0) {
                    $infantFare = ($flight1['infant_fare'] * $child);
                }
                ...
                if(array_key_exists($flight1['legs'][0]['Carrier_code'],$admin_markup)){
                    ...
                    if($admin_markup['type'] == 'fixed'){
                        ...
                        $admMark = ((($admin_markup[$airCode]/100)*$outTotalFare)/2);
    ...
    public function booking(){
        ...
        if(isset($post['flight_id']) && isset($post['return_id'])){
            ...
            if(trim($searchParms['journey_type']) == 'one_way'){
                ...
                $masterId = $this->Flight_Model->addFlightBookingDetails($post,$onwardDetails,$searchParms,$priceFinalData,$agentData,$_SESSION[session_id()]['admin_markup']);
                $booking = $this->Via_Model->booking($post,$onwardDetails,$searchParms,$priceFinalData,$requestId,$masterId);
                if($booking['status'] != 'Failed'){
                    $this->Home_Model->deductAgentDepositOnBooking($_SESSION['agent']['user_id'],$_SESSION['agent']['user_type'],$booking['fare'],$_SESSION[session_id()]['admin_markup']);
                ...
                $priceFinalData = $_SESSION[session_id()]['Via_booking_final'];
                $masterId = $this->Flight_Model->addFlightBookingDetailsRound($onwardDetails,$returnDetails,$post,$searchParms,$priceFinalData,$agentData,$_SESSION[session_id()]['admin_markup']);
