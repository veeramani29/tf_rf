...
class Home_Model extends CI_Model {
    ...
    function getFlightAutoFillData($input) {
        $sql = "select * from airport_list where (airport_name like '%$input%' or airport_code like '%$input%') order by airport_code desc, airport_city desc ";
...
//        $sql1 = "select * from hotelspro_city where (City like '%".$input."%' or Country like '%".$input."%') order by City asc,Country asc";
//        $query1=$this->db->query($sql1);
//        if($query1->num_rows() > 0)
//        {
//             $hotelsPro = $query1->result_array();
//        }
//        else $hotelsPro = '';
        $hotelsPro = '';
        $list = array('roomsxml' => $roomsxml, 'hotelsPro' => $hotelsPro);
    ...
    function getUserDetails($id) {
        $query = $this->db->query($sql = "select user_id,user_email,user_password,user_logo,first_name,company_name,phone_code,mobile_no,address,pin_code,city,country from agent_info where user_id = '" . $id . "'");
        if ($query->num_rows() > 0) {
            $data = $query->row();
            return $data;
        } else
