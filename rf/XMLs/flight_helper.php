...
<air:InitialAsynchResult   MaxWait="10000"/>
    ...
    <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
        ...
        <s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
            <'.$search_method.' SolutionResult="true" PreferCompleteItinerary="true" AuthorizedBy="user" TargetBranch="P7003720" xmlns:air="http://www.travelport.com/schema/air_v33_0">
                <com:BillingPointOfSaleInfo OriginApplication="UAPI" xmlns:com="http://www.travelport.com/schema/common_v33_0" />
                    ...
                    '.$child_patch.'
                    '.$infant_patch.'
                    <air:AirPricingModifiers  CurrencyType="EUR"></air:AirPricingModifiers>
                     '.$AsynchModifiers.'
            </'.$search_method.'>
            </s:Body>
    </s:Envelope>';
  #FaresIndicator="PublicAndPrivateFares"
     ...
     $LowFareSearchRes = processRequest($LowFareSearchReq);
      /*echo $LowFareSearchRes;exit;process_Profile_Request
...
<soapenv:Header/>
<soapenv:Body>
<air:RetrieveLowFareSearchReq AuthorizedBy="user" ProviderCode="'.$request->ProviderCode.'" SearchId="'.$request->SearchId.'" TargetBranch="P7003720" TraceId="trace">
...
$AirSegmentPricingModifiers.='<AirSegmentPricingModifiers AirSegmentRef="'.$segment->AirSegment_Key.'" FareBasisCode="'.$segment->FareBasis.'" ></AirSegmentPricingModifiers>';
    ...
    if($request->CHD != 0){
        for ($i=0; $i < $request->CHD; $i++) { 
            $childs .= '<SearchPassenger Key="'.$paxId.'" Code="'.$ChildKey.'" Age="10" xmlns="http://www.travelport.com/schema/common_v33_0" ></SearchPassenger>';
            $paxId++;
        }
    }
    $infants = '';
...
//$flight_segments[0]->PlatingCarrier
    ...
    $payment='';
     if($segment->ProviderCode=="ACH"){
          ...
          <CreditCard CVV="737" ExpDate="2018-08" Name="Veera K" Number="4111111111111111" Type="VI">
          <BillingAddress>
    ...
    <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
        ...
        <s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
            <AirPriceReq TargetBranch="P7003720" CheckOBFees="true" AuthorizedBy="user" xmlns="http://www.travelport.com/schema/air_v33_0" xmlns:common_v33_0="http://www.travelport.com/schema/common_v33_0">
 ...
 $AirItinerary_xml = str_replace($array_sr,'',$AirItinerary_xml);
    ...
    $Passport_required=false;
      ...
      while ($APISRequirements->length > 0) {
        ...
        foreach ($APISRequirements as $APISRequirement) {
$Doc_types = $APISRequirements->item($api)->getElementsByTagName('Document');
$docinc=0;
foreach ($Doc_types as $Doc_type) {
$document_type=$Doc_types->item($docinc)->getAttribute('Type');
$document_Level=$Doc_types->item($docinc)->getAttribute('Level');
if($document_type=='Passport' && $document_Level='Required'){
...
$docinc++;
             ...
             $node = $APISRequirements->item($api);
             remove_node($node);
             $api++;
        }
...
$prefix='Prefix="'.$checkout_form->$title.'"'; $SSR_a_pass_req='';$SSR_c_pass_req='';$SSR_pass_req='';$SSR_i_pass_req='';


if($Passport_required){
   

            $SSR_pass_req.= ' <SSR  FreeText="P/'.$checkout_form->country.'/AF'.mt_rand(100000,999999).'/'.$checkout_form->country.'/'.date('dMy',strtotime($doba)).'/'.substr($checkout_form->$gender,0,1).'/18Sep19/'.$checkout_form->$lname.'/'.$checkout_form->$fname.'"   Status="HK" Type="DOCS"/>';
        }
 ...
 $adults .= '<BookingTraveler Key="0" TravelerType="ADT" DOB="'.$doba.'" Nationality="'.$checkout_form->country.'" Gender="'.substr($checkout_form->$gender,0,1).'" xmlns="http://www.travelport.com/schema/common_v33_0">
                            ...
                            <PhoneNumber   Number="555-1212" Type="Home"/>
                      
                           <!-- this required for tiger air <com:PhoneNumber AreaCode="403" CountryCode="1" Location="YYC" Number="555-1212" Type="Home"/> -->
   ...
   $prefix_a='';
    if($request->ADT >1){
        for ($i=1; $i < $request->ADT; $i++) {
            ...
            $SSR_a_pass_req.= ' <SSR  FreeText="P/'.$checkout_form->country.'/FM'.mt_rand(100000,999999).'/'.$checkout_form->country.'/'.date('dMy',strtotime($doba)).'/'.substr($passengers->gender[$s]->$adult,0,1).'/18Sep19/'.$passengers->last[$s]->$audlt.'/'.$passengers->first[$s]->$audlt.'"   Status="HK" Type="DOCS"/>';
        }
    ...
    if($request->CHD != 0){
        ...
        $child="childs"."$ss";
         ...
         $dobc=$passengers->dob[$s]->$child;
          $prefix_c_split= ($provider=="ACH")?$passengers->title[$s]->$child:$passengers->title[$s]->$child;
         $prefix_c='Prefix="'.$prefix_c_split.'"'; 
     $address='';

     if($Passport_required){
   

            $SSR_c_pass_req.= ' <SSR  FreeText="P/'.$checkout_form->country.'/FA'.mt_rand(100000,999999).'/'.$checkout_form->country.'/'.date('dMy',strtotime($dobc)).'/'.substr($passengers->gender[$s]->$child,0,1).'/18Sep19/'.$passengers->last[$s]->$child.'/'.$passengers->first[$s]->$child.'"   Status="HK" Type="DOCS"/>';
        }
        $TravelerType= ($provider=="ACH")?"CHD":"CNN";
              $childs .= '<BookingTraveler Nationality="'.$checkout_form->country.'" Key="'.$paxId.'" TravelerType="'.$TravelerType.'"   DOB="'.$dobc.'" Gender="'.substr($passengers->gender[$s]->$child,0,1).'" xmlns="http://www.travelport.com/schema/common_v33_0">
  ...
  $dobi=$passengers->dob[$s]->$infant;
            ...
            $SSR_i_pass_req.= ' <SSR  FreeText="P/'.$checkout_form->country.'/IF'.mt_rand(100000,999999).'/'.$checkout_form->country.'/'.date('dMy',strtotime($dobi)).'/'.substr($passengers->gender[$s]->$infant,0,1).'/18Sep19/'.$passengers->last[$s]->$infant.'/'.$passengers->first[$s]->$infant.'"   Status="HK" Type="DOCS"/>';
        }

$address='';
              $prefix_i=($provider=="ACH")?"Mr":"Mstr";
            //$prefix_c='Prefix="'.$passengers->title[$s]->$child.'"'; 
            $infants .= '<BookingTraveler Key="'.$paxId.'" Nationality="'.$checkout_form->country.'" TravelerType="INF" DOB="'.$dobi.'" Gender="'.substr($passengers->gender[$s]->$infant,0,1).'" xmlns="http://www.travelport.com/schema/common_v33_0">
                            <BookingTravelerName Prefix="'.$prefix_i.'" First="'.$passengers->first[$s]->$infant.'"  Last="'.$passengers->last[$s]->$infant.'" ></BookingTravelerName>
                            <PhoneNumber Number="'.$checkout_form->mobile.'" Type="Mobile" ></PhoneNumber>
                            <Email EmailID="'.$checkout_form->email.'" Type="P" ></Email>
                             '.$SSR_i_pass_req.'   
                             '.$eachaddress.'   
 ...
 //echo $adults.$childs.$infants;die;
    ...
    <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                        <s:Header/>
                        <s:Body>
                            <univ:AirCreateReservationReq xmlns:common_v33_0="http://www.travelport.com/schema/common_v33_0" xmlns:univ="http://www.travelport.com/schema/universal_v33_0" AuthorizedBy="user" TargetBranch="P7003720" xmlns="http://www.travelport.com/schema/air_v33_0" RetainReservation="Both">
...
<soapenv:Header/>   
<soapenv:Body> 
<gds:GdsQueuePlaceReq AuthorizedBy="user" ProviderCode="1G" ProviderLocatorCode="'.$ProviderLocatorCode.'" PseudoCityCode="35AU" TargetBranch="P7003720" TraceId="trace">    
<com:BillingPointOfSaleInfo OriginApplication="UAPI"/>    
...
<gds:GdsEnterQueueReq xmlns:com="http://www.travelport.com/schema/common_v34_0" xmlns:gds="http://www.travelport.com/schema/gdsQueue_v34_0" AuthorizedBy="user" ProviderCode="1G" ProviderLocatorCode="'.$ProviderLocatorCode.'" PseudoCityCode="6I90" TargetBranch="P7003720"> 
<com:BillingPointOfSaleInfo OriginApplication="UAPI"/> 
...
<soapenv:Header/>  
<soapenv:Body>    
<air:AirTicketingReq xmlns:air="http://www.travelport.com/schema/air_v33_0" AuthorizedBy="user" BulkTicket="false" ReturnInfoOnFail="true" TargetBranch="P7003720" TraceId="trace">       
...
function CancelReq($LocatorCode){
    ...
    $CancelReq = '<?xml version="1.0" encoding="UTF-8"?>
                    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:com="http://www.travelport.com/schema/common_v33_0" xmlns:univ="http://www.travelport.com/schema/universal_v33_0">
                       <soapenv:Header />
                       <soapenv:Body>
                          <univ:UniversalRecordCancelReq AuthorizedBy="user" TargetBranch="P7003720" TraceId="trace" UniversalRecordLocatorCode="'.$LocatorCode.'" Version="1">
                             <com:BillingPointOfSaleInfo OriginApplication="UAPI" />
...
function process_Profile_Request($request,$service=''){
...
if(isset($credentials->username))
{
    ...
    $auth = base64_encode('Universal API/'.$credentials->username.':'.$credentials->password);        
   

    $soapAction = "";
    $UApi_URL='https://twsprofiler.travelport.com/Service/Default.ashx/AirService';
    $header = array( 
    "Connection:close",
