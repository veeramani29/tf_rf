...
if(strtotime($request->depart_date)==strtotime($city_b)) { $fare_cal.= '<td class="bg-primary"> '.lang('DEPART').' '.$city_b.'</td>'; } else { $fare_cal.= '<td class="bg-info"> '.lang('DEPART').' '.$city_b.'</td>'; } // city_a ad
}
$fare_cal.='</tr>';

foreach(array_keys($result) as $city_a) { // need the city_a as row index
$fare_cal.='<tr>';
if(strtotime($request->return_date)==strtotime($city_a)) { $fare_cal.= '<td class="bg-primary"> '.lang('RETURN').' '.$city_a.'</td>'; } else { $fare_cal.= '<td clas="bg-warning" > '.lang('RETURN').' '.$city_a.'</td>'; } // city_a ad
	...
	if($request->type != 'M' && $flex_air_segment_date!=''){
		...
		<div class="chart">
			...
			<ul class="legend list-unstyled">
				<li><span class="lowest_fare"></span><?php echo lang('TBL_RSLT_LEG');?></li>
				<li><span class="selected_date"></span><?php echo lang('TBL_RSLT_LEG1');?></li>
				<li><span class="normal_fare"></span><?php echo lang('TBL_RSLT_LEG2');?></li>
				<li><?php echo lang('TBL_RSLT_LEG3');?></li>
								...
								$(document).ready(function(){
									...
									$('.colr-he1').click(function(){
										...
										if($('#trip_type').val()=='oneway'){
												$('#hidden_from').val($.datepicker.formatDate('dd-mm-yy', new Date(date1)));
										}else{
