...
class B2c extends CI_Controller {
	...
	$data['url'] = WEB_URL . 'b2c/login';
        if ($status == 2) {
           ...
           /* $message = 'Your Account Has Been Closed. Please Contact Admin<br><br>';
            ...
            $subject = 'InnoGlobe - Your Account Closed';
	   ...
	   $this->Email_Model->send_add_user_active($data);
		...
		$data['email_template'] = $this->Email_Model->get_email_template('add_user_active')->row();
            ...
            if ($status == 0) {
                /*$message = 'Your Account Has Been DeActivated. Please Contact Admin<br><br>';
			...
			<strong>Name</strong> : ' . $fnam . '<br>
                ...
                $subject = 'InnoGlobe - Your Account DeActivated';
		 $this->Email_Model->send_email($fnam, $subject, $email3, $message_header, $message);*/
    ...
    function update_user($id) {
        ...
        $postal = $_POST['postal'];
        $domain = $_POST['domain'];
        if ($this->B2c_Model->update_b2c_user($id, $sal, $fnam, $lname, $address, $phone, $city, $country, $postal, $domain)) {
            redirect('b2c/b2c_view', 'refresh');
        } else {
            $data['status'] = '<div class="alert alert-block alert-danger">
							  <a href="#" data-dismiss="alert" class="close">×</a>
