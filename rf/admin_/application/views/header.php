...
<header>
      <nav class='navbar navbar-default navbar-fixed-top'>
        <a class='navbar-brand' href='<?=base_url();?><?php if($this->session->userdata("admin_logged_in")){echo"sdadmin";}?>'>
          <img style="height:75%;" class="logo" alt="Flatty" src="<?=base_url();?>assets/images/logo.jpg" />
        ...
        <!--<li class='dropdown medium only-icon widget'>
         ...
         <li class='dropdown medium only-icon widget'>
            <a class='dropdown-toggle has-tooltip' data-toggle='dropdown' title="B2B Support Ticket Inbox" data-placement='bottom' href='#'>
              <i class='icon-group'></i><i class='icon-ticket'></i>
              <div class='label'><?php echo $bs_t['cnt']; ?></div>
            </a>
            <ul class='dropdown-menu'>
            <?php 
			if($bs_t['result']!='') { foreach($bs_t['result'] as $res) { ?>
              <li>
