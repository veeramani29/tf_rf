		...
		if($count == 1){ //if count ='s one then move fwd
			...
			if($userInfo->status == 1){   
				...
				$response = array(
					'status' => '0',
					'success' => 'false',
					'msg' => "Invalid password. If you previously logged in with ".$userInfo->loggedin_with.", click 'Log in with ".$userInfo->loggedin_with."' to access your rfactory account."
				);
			}
		}else{
			$response = array(
