...
class Booking extends CI_Controller {
    ...
    public function index($session_id='',$orderIds=''){
        ...
        parent::pre_url();
           // echo $continue = $this->session->userdata('continue');
        if(!empty($session_id) && empty($orderIds)){
            ...
            if($count > 0){
                ...
                $this->load->view('apartment/booking-details', $data);
            ...
            if($count > 0){
                ...
                $tem_data=$data['pnr_nos'][0];
                
                $a=$this->cart_model->get_booking($tem_data->ref_id,$tem_data->module)->result();
                
                $cart_global_id[] = $tem_data->module.','.$a[0]->ID;
    ...
    public function checkBillingDetailsB2c($user_id) {
        ...
        if($billing_details->billing_firstname == "" || $billing_details->billing_lastname == "" || $billing_details->billing_addressA == "" || $billing_details->billing_email == "" || $billing_details->billing_contact == "" || $billing_details->billing_country == "" || $billing_details->billing_city == "" || $billing_details->billing_home_no == "" || $billing_details->billing_postal == "") {
            return true;
        } else {
            return false;
        }
    ...
    public function docontinue($Total, $Email, $OrderId){
...
//AMOUNT=1500Mysecretsig1875!?CURRENCY=EURMysecretsig1875!?LANGUAGE=en_USMysecretsig1875!?
    ...
    public function doPaymentGate($Total, $OrderId, $Email ){
           // print_r($_REQUEST);die;
           
           $Total = json_decode(base64_decode($Total));
           if($this->input->post('payment_type')=='CreditCard'){
            $perc_Total=(($Total/100)*2);   // 2%
             $Totals=($Total+$perc_Total);
            ...
            $data['params'] = array(
                ...
                'TBLBGCOLOR' => '#FFF',
                'TBLTXTCOLOR' => '#66AFE9',
                'TITLE' => 'To Pay ReservationFactory',
                'TXTCOLOR' => '#FFF',
...
$data['params']['sha1']=sha1($sha1);
    ...
    public function account_create_booking($data,$psssword){
        $postData = array(
            ...
            'loggedin_with' => 'rfactory'
        );
        $count = $this->account_model->isRegistered($data['billing_email'])->num_rows();
        if($count == 0){
            $this->account_model->createUsers($postData);
            $user_data = $this->account_model->isRegistered($data['billing_email'])->row();
            $b2c_session =  array(
    ...
    public function checkout(){
       ...
       // $checkout_form['address2']='';
        $c=json_decode(base64_decode($checkout_form['cid']));
        
        $cart_id=explode(",", $c[0]);
        $cart_id=$cart_id[1];
       
        $billing['billing_firstname'] =$checkout_form['first_name'.$cart_id];
        $billing['billing_lastname'] =$checkout_form['last_name'.$cart_id];
        ...
        $billing['billing_mobilecountrycode'] = $checkout_form['country_code'];
        $billing['billing_city'] = $checkout_form['city'];
        $billing['billing_home_no'] = $checkout_form['housenum'];
        $billing['billing_postal'] = $checkout_form['zip'];
        
        if(isset($checkout_form['txtPassword'])){

            if($checkout_form['txtPassword']!=null && $checkout_form['txtcPassword']!=null){
            	 $psssword = $checkout_form['txtPassword'];
                $account=$this->account_create_booking($billing,$psssword);
                if(!$account){

                     $continue_url = $this->session->userdata('continue');
                     echo "<script language=\"javascript\">if(confirm('This email address is already in use')){ alert('Please continue without account creation'); window.location.replace('".$continue_url."') ; }else{ window.location.replace('".$continue_url."') }</script>";
                      die;
              }
        ...
        foreach ($cids as $key => $cid) {
            ...
            if($module == 'FLIGHT'){
                ...
                if(isset($checkout_form['email']) && $checkout_form['email']!=''){
                	$guest_email=$checkout_form['email'];
                }else{
                	$guest_email=$checkout_form['email'];
                }

                if(isset($checkout_form['mobile']) && $checkout_form['mobile']!=''){
                	 $country_code = $this->apartment_model->get_country_phonecode($checkout_form['country_code']);
                ...
                if(isset($checkout_form['selTitle'.$cid]) && $checkout_form['selTitle'.$cid]!=''){
                    $guest_title=$checkout_form['selTitle'.$cid];
                }else{
                    $guest_title='';
                }
                ...
                $dd=$checkout_form['birth_day'.$cid]; $mm=$checkout_form['birth_month'.$cid];$yyyy=$checkout_form['birth_year'.$cid];

                if($dd!='' && $mm!='' && $yyyy!=''){
                	$guest_dob=$yyyy.'-'.$mm.'-'.$dd;
                }else{
                	$guest_dob='';
                }

                $pass_result=json_decode(base64_decode($checkout_form['passengers']));
                $db_array["gender"]=array();$db_array["first"]=array();$db_array["last"]=array();$db_array["dob"]=array();$db_array["title"]=array();
                $dob_Arr=array();
                if(isset($checkout_form["pfirst_name$cid"])){
                 ...
                 $title_Arr=$checkout_form["pselTitle$cid"];
				 $date_of_year_Arr=$checkout_form["pbirth_year$cid"];
                 $date_of_month_Arr=$checkout_form["pbirth_month$cid"];
                 $date_of_date_Arr=$checkout_form["pbirth_day$cid"];

                 $date_of_year_Arr_values=array_values($checkout_form["pbirth_year$cid"]);
                 $date_of_year_Arr_keys=array_keys($checkout_form["pbirth_year$cid"]);
                 $date_of_month_Arr_values=array_values($checkout_form["pbirth_month$cid"]);
                 $date_of_date_Arr_values=array_values($checkout_form["pbirth_day$cid"]);

                  for ($i=0; $i <count($date_of_year_Arr_values); $i++) { 
                    $dob_Arr[]=array("$date_of_year_Arr_keys[$i]"=>$date_of_year_Arr_values[$i]."-".$date_of_month_Arr_values[$i]."-".$date_of_date_Arr_values[$i]);
                  }
                 ...
                 foreach($title_Arr as $key=>$value){
                     $db_array["title"][]=array("$key"=>$value);
                 }
			 }
		      ...
		      $passenger_full_details=base64_encode(json_encode($db_array));
				$booking_flight = array(
                    ...
                    'passenger_details' => $checkout_form['passengers'],
                     'passenger_full_details' => $passenger_full_details,
                      'passenger_price_details' => $checkout_form['passengers_with_price'],
					'BILLING_FIRSTNAME' => $guest_fname,
                   // 'BILLING_MIDDLENAME' => $checkout_form['middle_name'],
                    'BILLING_LASTNAME' => $guest_lname,
                    ...
                    'BILLING_HOME_NO' => $checkout_form['housenum'],
                    'BILLING_ZIP' => $checkout_form['zip'],
					'TotalPrice' => ($TOTAL+$cart_flight_data->HiddenMarkupDiscount),
					'DISCOUNT' => $DISCOUNT,
					'SITE_CURR' => $cart_flight_data->SITE_CURR,
					'API_CURR' => $cart_flight_data->API_CURR,
                    'MyMarkup' => $cart_flight_data->MyMarkup,
                     'HiddenMarkupDiscount' => $cart_flight_data->HiddenMarkupDiscount,
                    'VissibleOthersMarkup' => $cart_flight_data->VissibleOthersMarkup,
				...
				//echo '<pre>';print_r($booking_flight);die;
                ...
                $pnr_no = 'RF'.date('m').'F'.date('dHi').$bid;
            ...
            $hotel_addresses = base64_decode($checkout_form['hotel_addresses']);
            if(isset($checkout_form['first_name'.$cid]) && $checkout_form['first_name'.$cid]!=''){
                $guest_fname=$checkout_form['first_name'.$cid];
            }else{
                $guest_fname=$checkout_form['first_name'];
           ...
           // if($cart_hotel_data->api != 'CRS'){
                ...
                $booking_hotel = array(
                    ...
                    'cid' =>$cid,
                    'search_request' => $checkout_form['request_data'],
                   ...
                   // 'BILLING_STATE' => $checkout_form['state'],
					...
					'USER_TYPE' => $user_type,
                    'USER_ID' => $user_id,
					'hotel_addresses' => $hotel_addresses,
					'TIMESTAMP' => date('Y-m-d H:i:s')
                );
				...
				$booking = array(
                    ...
                    'user_type' => $user_type,
                    'amount' => $TOTAL,
                    'leadpax' => $guest_fname.' '.$guest_lname,
                    'user_id' => $user_id,
                    'ip' =>  $this->input->ip_address(),
					'payment_method' => $payment_method
                );
                ...
                $pnr_no = 'RF'.date('m').'H'.date('dHi').$bid;
                $update_booking = array(
                    'pnr_no' => $pnr_no,
					'booking_status' => 'PROCESS',
					'transaction_status' => 'PROCESS'
            ...
            $country_code1 = $this->apartment_model->get_country_phonecode($checkout_form['country_code']);
                ...
                $cart_car_data = $this->car_model->getBookingTemp($cid)->row();
                ...
                $pnr_no = 'RF'.date('m').'C'.date('dHi').$bid;
                $update_booking = array(
                    'pnr_no' => $pnr_no,
					'booking_status' => 'PROCESS',
					'transaction_status' => 'PROCESS'
               ...
               $pnr_no = 'RF'.date('m').'V'.date('dHi').$bid;
                $update_booking = array(
                    'pnr_no' => $pnr_no,
					'booking_status' => 'PROCESS',
					'transaction_status' => 'PROCESS'
        ...
        if($this->session->userdata('b2b_id')){
			...
			*/
            ...
            $OrderId = base64_encode(json_encode($OrderId));
              ...
              $GateURL = WEB_URL.'/booking/docontinue/'.$TOTAL.'/'.$Email.'/'.$OrderId;
             redirect($GateURL);
           // $data['status'] = 555;
           // $data['GateURL'] = $GateURL;
				...
				if($book_check=='ok'){
					...
					if($module == 'FLIGHT'){
						...
						if($count == 1){
                            ...
                            $update_booking = array(
                            'booking_no' => $SupplierLocatorCode,
							...
							'booking_no' => $booking_res->LocatorCode,
							'api_status' => $Status,
                            'ReservationLocatorCode' => $AirReservationLocatorCode,
							'booking_status' => $BookingStatus
							);*/
                            ...
                            // GdsQueue Availability Start
                              if ($booking_res->BookingStatus == 'CONFIRMED') {
                               

$GdsQueuePlaceReq_Res = GdsQueuePlaceReq($ProviderLocatorCode);

$GdsQueuePlaceRes = $GdsQueuePlaceReq_Res['GdsQueuePlaceRsp'];
$GdsQueuePlace = $this->xml_to_array->XmlToArray($GdsQueuePlaceRes);
if(isset($GdsQueuePlace['SOAP:Body']['gdsQueue:GdsQueuePlaceRsp'])){
...
$this->flight_model->Update_Booking_Global($bid, $update_booking, 'FLIGHT');
                    ...
                    $xml_log = array(
                        'Api' => 'UAPI',
                        'XML_Type' => 'FLIGHT - GdsQueuePlaceRsp  EMPTY - ERROR',
                        'XML_Request' => $GdsQueuePlaceReq_Res['GdsQueuePlaceReq'],
                        'XML_Response' => $GdsQueuePlaceReq_Res['GdsQueuePlaceRsp'],
                        'Ip_address' => $this->input->ip_address(),
                        'XML_Time' => date('Y-m-d H:i:s')
                    );
                    $this->xml_model->insert_xml_log($xml_log);
                              ...
                              // GdsQueue Availability Start End
                        ...
                        $this->flight_mail_voucher_send($pnr_no);
					...
					if($module == 'HOTEL')
					{
						...
						if($count == 1)
						{
							...
							$checkout_form = json_decode($book_temp_data->TravelerDetails);
							 $cid = $book_temp_data->cid;
							$payment_result=$global_id->payment_res;
							$returndata = make_hotel_booking($cart_hotel_data, $cid, $checkout_form,$payment_result);
                           // echo $returndata['response'];	
							//die;
							...
							'LocatorCode' => $LocatorCode,
							'ProviderLocatorCode' => $ProviderLocatorCode,
							'ReservationLocatorCode' => $HotelReservationLocatorCode,
					...
					if($module == 'CAR')
					{
                        ...
                        //print_r($global_id->payment_res);die;
						...
						if($count == 1)
						{
							...
							$cid = $book_temp_data->cid;
                            $payment_result=$global_id->payment_res;
							$VehicleCreateReservationReq_Res = $res = VehicleCreateReservationReq($cart_car_data, $checkout_form, $cid,$payment_result);
				...
				$VehicleCreateReservationReq = $VehicleCreateReservationReq_Res['VehicleCreateReservationReq'];
					...
					$VehicleBookRS = str_replace('common_v31_0:','',$VehicleBookRS);
    ...
    public function flight_mail_voucher($pnr_no){
     ...
     public function flight_mail_voucher_send($pnr_no){
