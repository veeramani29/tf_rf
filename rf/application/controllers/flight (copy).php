...
class Flight extends CI_Controller {
	...
	public function search_flight(){
		...
		$request['Airline']=$post_request['Airline'];
...
if($flight!='' && $request!=''){
		...
		if(isset($AirPriceRes->Body->AirPriceRsp->AirPriceResult)){
			...
			$cart_data = $this->cart_model->getCartData($session_id)->result();
			 ...
			 $AirPriceErrorRes = str_replace('common_v28_0:','',$AirPriceErrorRes);
	...
	function formatResponse($LowFareSearchReq_Res, $aMarkup, $myMarkup){
	   ...
	   // echo '<pre>';print_r($LowFareSearchRes);die;
		...
		{
	    $CurrencyType = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['@attributes']['CurrencyType'];
	    $FlightDetails = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:FlightDetailsList']['air:FlightDetails'];
	    $AirSegment = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:AirSegmentList']['air:AirSegment'];
	    $AirPricingSolutions = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:AirPricingSolution'];
	    $FareInfos = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:FareInfoList']['air:FareInfo'];
	     $FareBasis = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:FareInfoList']['air:FareInfo'];
	     if(isset($LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:HostTokenList'])){
	    $HostTokenLists = $LowFareSearchRes['SOAP:Body']['air:LowFareSearchRsp']['air:HostTokenList']['common_v28_0:HostToken'];
	}
						...
						foreach ($AirSegmentRefs as $jk => $AirSegmentRef) {
							...
							foreach ($AirSegments as $key => $AirSegmentt) {
								...
								if($AirSegment_Key == $AirSegmentRef_Key){
									...
									$fs[$i][$mode]['segments'][$jk]['ParticipantLevel'] = $ParticipantLevel = $AirSegment_Attr['ParticipantLevel'];
					...
					foreach ($AirSegmentRefs as $jk => $AirSegmentRef) {
						...
						foreach ($AirSegments as $key => $AirSegmentt) {
							...
							if($AirSegment_Key == $AirSegmentRef_Key){
								...
								$fs[$i]['segments'][$jk]['ParticipantLevel'] = $ParticipantLevel = $AirSegment_Attr['ParticipantLevel'];
								if(isset($AirSegment_Attr['APISRequirementsRef'])){
...
$Farerulesref_Key=array();$Farerulesref_Provider=array();$Farerulesref_content=array();
	...
	function formatMultiResponse($LowFareSearchReq_Res, $aMarkup, $myMarkup){
		...
		$tag_value =$exp_fare[2];
		 ...
		 $fare_message = <<<EOM
			<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
				<soapenv:Header/>
				<soapenv:Body>
					<ns2:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v29_0" xmlns:ns2="http://www.travelport.com/schema/air_v29_0" FareRuleType="long" TargetBranch="P7003720" AuthorizedBy="Test">
...
EOM;
		...
		$fareRule = $xmlDoc->getElementsByTagNameNS('http://www.travelport.com/schema/air_v29_0','FareRuleLong');
...
$plot_data=array();
	...
	if($flex_file != 0){
		...
		$flex_air_pricing = $flexXmlDoc->getElementsByTagNameNS('http://www.travelport.com/schema/air_v28_0','AirPricingSolution');
		foreach ($flex_air_pricing as $flex_air_pricing_block){
	        $flex_price[] = $flex_air_pricing_block->getAttribute('TotalPrice');
		}
		$flex_air_segment = $flexXmlDoc->getElementsByTagNameNS('http://www.travelport.com/schema/air_v28_0','AirSegment');
		...
		$flex_round_air_segment = $flexRoundXmlDoc->getElementsByTagNameNS('http://www.travelport.com/schema/air_v28_0','AirSegment');
		...
		$flex_round_air_pricing = $flexRoundXmlDoc->getElementsByTagNameNS('http://www.travelport.com/schema/air_v28_0','AirPricingSolution');
		foreach ($flex_round_air_pricing as $priceblock) {
	        $flex_round_price[] = $priceblock->getAttribute('TotalPrice');
	        $dir=0;
	        $journeyblock = $priceblock->getElementsByTagNameNS('http://www.travelport.com/schema/air_v28_0','Journey');		
			foreach ($journeyblock AS $jblock){
				$seg_index=0;
				$flex_round_air_segment_ref = $jblock->getElementsByTagNameNS('http://www.travelport.com/schema/air_v28_0','AirSegmentRef');
