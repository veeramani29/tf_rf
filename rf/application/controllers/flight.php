...
class Flight extends CI_Controller {
	...
	public function search_flight(){
		...
		$post_request['search_data_expiry']=date("H:i", strtotime('5 minutes', time()));
		$this->session->set_userdata('search_data',$post_request);
...
//if($c_user_id!='' && $c_user_type!='' && $data['Booking']->user_id == $c_user_id && $data['Booking']->user_type == $c_user_type)
	...
	public function AddToCart(){
		...
		elseif($request->type == 'R') {
			$AllSegments = $flight->onward->segments;
...
$WholeMarkupMaxAmount = $WholeMarkup[7];
	 ...
	 if($WholeMarkup[0] == 'Addition') {
		...
		if(isset($AirPriceRes->Body->AirPriceRsp->AirPriceResult)){
	        ...
	        if($cart->TotalPrice!=$flight->TotalPrice){
...
$this->session->set_flashdata('price_change_msg', 'Your selected flight price has been slightly '.$change_type.'  changed. Due to API real time data.');
...
$more_results['SearchId']=$SearchId;
    ...
    public function all_cancellation(){
            ...
            if($b_data->booking_status == 'CONFIRMED'){
			    ...
			    if(isset($CancelRes['SOAP:Body']['universal:UniversalRecordCancelRsp'])){
			    	...
			    	if($CancelResAttr['Cancelled']){
			    		...
			    		$data['suc_msg']="Successfully Cancelled your booking";
			    }else{
			    	$xml_log = array(
			    		'Api' => 'UAPI',
			    		'XML_Type' => 'FLIGHT - CANCELLATION -UniversalRecordCancelRsp EMPTY - ERROR',
          	...
          	$data['suc_msg']="Cannot found your booking no";  
        }
    }
		...
		$this->load->view('all_cancellation',$data);
    }
		...
		$tag_value =$exp_fare[2];
		 ...
		 $fare_message = <<<EOM
			<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
				<soapenv:Header/>
				<soapenv:Body>
					<ns2:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v33_0" xmlns:ns2="http://www.travelport.com/schema/air_v33_0" FareRuleType="long" TargetBranch="P1496502" AuthorizedBy="Test">
...
$plot_data=array();
 ...
 //===================delete data========================
 function delete_data($refid,$url){
  //echo trim($refid);

$this->db->where('parent_pnr',$refid);
$this->db->delete('booking_global'); 
