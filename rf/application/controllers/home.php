...
class Home extends CI_Controller {
	...
	public function language(){
		...
		if(isset($language_v) && $language_v!='' && isset($current_url) && $current_url!=''){
			...
			$this->lang->load('common', $language_v);
	...
	public function more_passangers(){
	$data=array();
	if($this->input->post()!=null){
	$post=$this->input->post();
	
	$lead=$post['lead_cont_name'];
	...
	$place=$post['place'];
	$Adddress=$address.",".$nr.",".$zip_code.",".$place;
	$phone=$post['phone'];
	...
	$from=($post['from']!=null)?$post['from']:'N/A';
	$to=($post['to']!=null)?$post['to']:'N/A';
	$departure_date=($post['departure_date']!=null)?$post['departure_date']:'N/A';
	$return_date=($post['return_date']!=null)?$post['return_date']:'N/A';
	$depature_time=($post['depature_time']!=null && $post['depature_min']!=null)?$post['depature_time'].":".$post['depature_min']:'N/A';
	$return_time=($post['return_time']!=null && $post['return_min']!=null)?$post['return_time'].":".$post['return_min']:'N/A';
	$flexible=($this->input->post('flexible')!=null)?$post['flexible']:'No';
	
	$total_adult=($post['total_adult']!=null)?$post['total_adult']:'N/A';
	$total_child=($post['total_child']!=null)?$post['total_child']:'0';
	$total_infant=($post['total_infant']!=null)?$post['total_infant']:'0';
	
	$total_passangesr="Adults :$total_adult, Childs :$total_child, Infants :$total_infant";
	
	$booking_reason=($post['booking_reason']!=null)?$post['booking_reason']:'N/A';
	$other_request=($post['other_request']!=null)?$post['other_request']:'N/A';
	
	 	
               
                $email_type = 'more_passanger_request';
                $datas = $this->Email_Model->get_email_template($email_type)->row();
                $template_message = $datas->message;
                $data['subject']= $datas ->subject;
                 $data['from']=$datas->email_from;
       $data['from_name']=$datas->email_from_name;
                 $order_mes=array("{%%NAME%%}","{%%CMPNY_NAME%%}","{%%EMAIL%%}",
        "{%%GENDER%%}","{%%PHONE%%}","{%%MOBILE%%}","{%%ADDRESS%%}","{%%PASSANGERS%%}",
        "{%%BOOKING_REASON%%}","{%%OTHER_REQ%%}","{%%FLEXIBLE%%}","{%%ORIGIN%%}","{%%DESTINATION%%}",
        "{%%DEPARTURE%%}","{%%RETURN%%}","{%%DEPARTURE_TIME%%}","{%%RETURN_TIME%%}","{%%WEB_URL%%}","##WEB_NAME##");
        $replace_mes=array($lead,$company,$email,$gender,$phone,$mobile,$Adddress,$total_passangesr,$booking_reason,$other_request,$flexible,
        $from,$to,$departure_date,$return_date,$depature_time,$return_time,WEB_FRONT_DIR,WEB_NAME);
        $message=str_replace($order_mes, $replace_mes, $template_message);

        $lang_order_mes=array("{%%TRAVELLER_BES_TFRND%%}","{%%MORE_PASSANGER_REQ%%}","{%%DEAR%%}",
            "{%%MORE_PASSANGER_REQ_MSG%%}","{%%LEAD_NAME%%}","{%%MAIL_CMPNY_NAME%%}","{%%MAIL_EMAIL%%}","{%%MAIL_GENDER%%}","{%%MAIL_PHONE%%}",
            "{%%MAIL_MOBILE%%}","{%%MAIL_ADDRESS%%}","{%%MAIL_PASSANGERS%%}","{%%MAIL_BOOKING_REASON%%}",
            "{%%MAIL_OTHER_REQ%%}","{%%MAIL_FLEXIBLE%%}","{%%TRIP_DETAILS%%}","{%%MAIL_ORIGIN%%}",
            "{%%MAIL_DESTINATION%%}","{%%MAIL_DEPARTURE%%}","{%%MAIL_RETURN%%}","{%%MAIL_DEPARTURE_TIME%%}",
            "{%%MAIL_RETURN_TIME%%}");
        $lang_replace_mes=array(lang_line('TRAVELLER_BES_TFRND'),lang_line('MORE_THAN_PERSON_REQ'),lang_line('DEAR'),
            lang_line('REQ_SUCC_MSG'),lang_line('lead_contact'),lang_line('company_name'),lang_line('More_Eail')
            ,lang_line('gender'),lang_line('tele_phone'),lang_line('mobile_num'),lang_line('address'),lang_line('More_Passangers'),lang_line('Booking_reason')
            ,lang_line('Other_Request'),lang_line('flexible'),lang_line('Your_trip'),lang_line('flying_from'),lang_line('flying_to'),lang_line('flying_from_date')
            ,lang_line('flying_to_date'),lang_line('flying_from_time'),lang_line('flying_to_time'));
            $data['message']=str_replace($lang_order_mes, $lang_replace_mes, $message);
            
                $Response = $this->Email_Model->sendmail_more_passangers($data);
              
               $data['suc_msg']=lang_line('REQ_SUCC_MSG');
             
	
	}
	
		
		$this->load->view('more_passangers',$data);	
	}
	...
	function get_airports(){
		...
		$airports = $this->flight_model->get_airport_list($term,$this->session->userdata['language']);
		...
		foreach($all_air as $airport){
			...
			if($splcity>2){
				...
				foreach($airport as $sec_airport){
					if($air==0){
				$apts1['label'] = $sec_airport['airport_city']." (All Airports)";
				//$apts1['label_name'] =$sec_airport['airport_name'].",".$sec_airport['country'];
				$apts1['label_name'] =$sec_airport['country'];
			 	$apts1['label_code'] ='('.$sec_airport['airport_city_code'].')';
				$apts1['value'] = $sec_airport['airport_city'].' ('.$sec_airport['airport_city_code'].') (All Airports)';
				$apts1['id'] = $sec_airport['airport_id'];
		 		$result[] = $apts1; 
					}else{
				$apts1['label'] = $sec_airport['airport_city'];
				$apts1['label_name'] =$sec_airport['airport_name'].",".$sec_airport['country'];
			 	$apts1['label_code'] ='('.$sec_airport['airport_code'].')';
