...
function LowFareSearchReq($request){
    ...
    if($request->type == 'O'){
    // echo $LowFareSearchReq_forflex;exit;
    $LowFareSearchRes_forflex = processRequest($LowFareSearchReq_forflex);
	$file = "001-1G_FlexRsp.xml";
	prettyPrint($LowFareSearchRes_forflex,$file);
}else{
...
function AirPriceReq($flight, $request){
    ...
    if($request->type == 'O' || $request->type == 'M'){
        foreach ($flight->segments as $key => $segment) {
            ...
            $segments .= '<AirSegment Key="'.$segment->AirSegment_Key.'" ClassOfService="'.$segment->BookingCode.'" ETicketability="'.$segment->ETicketability.'" Equipment="'.$segment->Equipment.'" ChangeOfPlane="'.$segment->ChangeOfPlane.'" AvailabilitySource="'.$segment->AvailabilitySource.'" OptionalServicesIndicator="'.$segment->OptionalServicesIndicator.'" PolledAvailabilityOption="'.$segment->PolledAvailabilityOption.'" Origin="'.$segment->Origin.'" Destination="'.$segment->Destination.'" DepartureTime="'.$segment->DepartureTime.'" FlightNumber="'.$segment->FlightNumber.'" Group="'.$segment->Group.'" Carrier="'.$segment->Carrier.'" ArrivalTime="'.$segment->ArrivalTime.'" ParticipantLevel="'.$segment->ParticipantLevel.'" LinkAvailability="'.$segment->LinkAvailability.'" ProviderCode="'.$segment->ProviderCode.'" FlightTime="'.$segment->FlightTime.'" Distance="'.$segment->Distance.'">
                            '.$CodeshareInfo.'
                            <AirAvailInfo ProviderCode="'.$segment->ProviderCode.'">
                                <BookingCodeInfo BookingCounts="'.$segment->BookingCounts.'"></BookingCodeInfo>
                            </AirAvailInfo>
                            <FlightDetails Equipment="'.$segment->Equipment.'"  '.$OriginTerminal.' '.$DestinationTerminal.' Destination="'.$segment->Destination.'" Origin="'.$segment->Origin.'" Key="'.$segment->FlightDetail_Key.'" FlightTime="'.$segment->FlightTime.'" ArrivalTime="'.$segment->ArrivalTime.'" DepartureTime="'.$segment->DepartureTime.'" ></FlightDetails>
                          </AirSegment>';
        }
...
function AirCreateReservationReq($cart_flight_data, $checkout_form, $cid){
	...
	$AirCreateReservationReq = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
                        <s:Header>
                            <Action s:mustUnderstand="1" xmlns="http://schemas.microsoft.com/ws/2005/05/addressing/none">http://localhost:8080/kestrel/AirService</Action>
                        </s:Header>
                        <s:Body xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
