...
class Flight_Model extends CI_Model {
	...
	public function get_airport_list($query,$lang='en'){
		...
		$this->db->like('airport_city', $query,'after'); 
		$this->db->or_like('country', $query,'after'); 
		$this->db->group_by("airport_city_code"); 
		$this->db->having('COUNT(*)>', 0); 
		$this->db->order_by("airport_id","asc");
		$f_q=$this->db->get($table);
		$tot_first=$f_q->num_rows();
		$air_tot=0;
		$city_tot=0;
 ...
 *  public function get_airport_details($code){
    ...
    public function get_discounted_flights($type){
			$qry="SELECT * FROM discounted_flights WHERE status='1' and offer_for='".$type."' and exp_date >= CURDATE()  limit 0,6";
			$data=$this->db->query($qry);
       //SELECT * FROM events WHERE event_date >= NOW()//CURDATE()
      // echo $this->db->last_query();
	...
	public function WholeMCalcCarrier($MCalcDepartureDate){
		$MarkupsQuery = "SELECT * FROM B2CMarkupDiscount 
						WHERE  ( ( VFrom <= '$MCalcDepartureDate' AND VTo >= '$MCalcDepartureDate' ) || ( VFrom = '0000-00-00' AND VTo = '0000-00-00' ) )
						AND Status = 'Active'
						";
