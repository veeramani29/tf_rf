...
$( "#cdepature" ).datepicker({
  ...
  beforeShowDay: function (date) {
    var myDate=$("#cdepature" ).val();
    myDate=myDate.split("/");
    var date1 =  new Date(myDate[2], myDate[1]-1, myDate[0]);
    var myDate=$("#creturn" ).val();
    myDate=myDate.split("/");
    var date2 = new Date(myDate[2], myDate[1]-1, myDate[0]);
...
$( "#creturn" ).datepicker({
	...
	maxDate: "+1y",
	onClose: function( selectedDate ) {		
		$( "#cdepature" ).datepicker( "option", "maxDate", selectedDate );
	},	
	beforeShowDay: function (date) {
		var myDate=$("#cdepature" ).val();
		myDate=myDate.split("/");
...
$( "#hotel_checkin" ).datepicker({
  ...
  beforeShowDay: function (date) {
    var myDate=$("#hotel_checkin" ).val();
    myDate=myDate.split("/");
    var date1 =  new Date(myDate[2], myDate[1]-1, myDate[0]);
    var myDate=$("#hotel_checkout" ).val();
    myDate=myDate.split("/");
    var date2 = new Date(myDate[2], myDate[1]-1, myDate[0]);
    if (date1!="" && date2!="" && date >= date1 && date <= date2) {
     return [true, 'highlightedDates-ui', ''];
...
$('#add-multiflight').click(function(){
  var MFCount = $('#mlti-flight-count').html();
  var MFCount = parseInt(MFCount) + parseInt(1);                    
  if(MFCount == 4){
    $('#add-multiflight').addClass('hidden');
  }
  if(MFCount <= 4){
    $('#mlti-flight-count').html(MFCount);
    if(MFCount > 0){
      $('#remove-multiflight').removeClass('hidden');
    }
    ManageMultiFlight(MFCount);
  }
});
$('#remove-multiflight').click(function(){
  var MFCount = $('#mlti-flight-count').html();
  var MFCount = parseInt(MFCount) - parseInt(1);
  if(MFCount <= 1){
    $('#remove-multiflight').addClass('hidden');
  }
  if(MFCount >= 1){
    $('#mlti-flight-count').html(MFCount);
    $('#add-multiflight').removeClass('hidden');
    if(MFCount > 1){
      $('#remove-multiflight').removeClass('hidden');
    }
    ManageMultiFlight(MFCount);
...
ManageMultiFlight(0);
function ManageMultiFlight(FlightCount){
  for (var i = 1; i <= FlightCount; i++) {
    $('#multiflight'+i).removeClass('hidden');
  };
  for (var i = parseInt(FlightCount) + parseInt(1); i <= 4; i++) {
    $('#multiflight'+i).addClass('hidden');
  };
}
