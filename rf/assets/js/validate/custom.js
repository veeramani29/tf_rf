...
$('#hotel_search').validate({ // initialize plugin within DOM ready
 // other options,
	    rules: {
	        'from[]': {
	            required: true
...
$('#car_search').validate({ // initialize plugin within DOM ready
 // other options,
	    rules: {
	        'from[]': {
	            required: true
	    ...
	    submitHandler: function() { 
	    	var action = $("#car_search").attr('action');
			$.ajax({
				type: "POST",
				url: action,
				data: $("#car_search").serialize(),
				//dataType: "json",
				beforeSend: function(XMLHttpRequest){
			    	$('.imgLoader').fadeIn();
...
$("#checkNewsLetter").on('click', function() {
	...
	$.ajax({
		...
		success: function(r) {
			...
			$('.nl_subs_loader').hide();
			if(setBit == 0) {
			$("#checkNewsLetter").attr("check",1);
				$('.ns_subd').fadeIn(100).fadeOut(5000);
				$("#checkNewsLetter").html("UnSubscribe");
				
			} else {
				$('.ns_unsub').fadeIn(100).fadeOut(5000);
				$("#checkNewsLetter").html("Subscribe");
				$("#checkNewsLetter").attr("check",0);
