...
class Discount extends CI_Controller {
	...
	if($this->session->userdata('admin_logged_in') || $this->session->userdata('sa_logged_in')){
		...
		if($id!=''){
			...
			$data['airports'] = $this->Discount_Model->get_airport_city_list();
	...
	public function why_we($id=''){




		$data['pages'] = $this->Discount_Model->get_why_we_details($id);
		$this->load->view('discount/manage_why_we',$data);
	}

	public function add_why_we($id=''){
		error_reporting(0);
        	...
        	$this->form_validation->set_rules('txtUrl', 'Short Descriptions', 'trim|required|min_length[3]|xss_clean');
			

        	
        	
        	
			if($this->input->post('why_we')=='submit'){
			...
			if($this->form_validation->run() == true){
				...
				$config['upload_path']='../uploads/why_we';
				$config['allowed_types'] = 'gif|jpg|png|jpeg';
				$config['max_size'] = '1000';
				$config['max_width']  = '';
				$config['max_height']  = '';
			...
			$data=array(
				...
				if($id!=''){
				$this->db->where('id', $id);
				$this->db->update('why_we', $data); 
				$flight_id=$id;
				//echo $this->db->last_query();exit;
			}else{
				$this->db->insert('why_we', $data);
				$flight_id=$this->db->insert_id();
			}
			
				redirect('discount/why_we');
			}else{
		...
		if($id!=''){
		$data['why_we_details'] = $this->Discount_Model->get_why_we_details($id);

			}
		$this->load->view('discount/add_why_we',$data);
	}
	...
	public function isTitleExists(){
		...
		$exists = $this->Discount_Model->isTitleExists($data);
		if($exists){
	...
	public function why_we_delete($id){
		$res = $this->Discount_Model->why_we_delete($id);
		if($res){
			redirect('discount/why_we','refresh');
		}
	}
	public function why_we_updateStatus($id,$value){
		$status = array('status'=>$value);
		$res = $this->Discount_Model->why_we_updateStatus($id,$status);
		if($res){
			redirect('discount/why_we','refresh');
		}
	}
