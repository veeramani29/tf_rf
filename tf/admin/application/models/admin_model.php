...
*/
    ...
    public function getsocial_links(){
     ...
     public function insurance_settings_update($values,$id){
		if($id){
			$this->db->trans_start();
			$this->db->update('insurance_settings', $values, "id = $id");
			$this->db->trans_complete();
			return $this->db->trans_status();
		}
    }
    public function getinsurance_settings(){
		$query = $this->db->get('insurance_settings', 1, 0);
		return $query->row();
    }
