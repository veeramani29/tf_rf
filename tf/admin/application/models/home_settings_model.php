...
class Home_Settings_Model extends CI_Model {
	...
	public function getAllHeaderLinks(){
		$query = $this->db->get('header_links');
		if ( $query->num_rows > 0 ) {
         		return $query->result();
      		}
      		return false;
	...
	public function get_check_in_details($id=''){

		if($id!=''){
			$this->db->where('id', $id);
		}
		
		$f_results=$this->db->get('check_in_details');
		
		return $f_results->result();
	}
	
	public function check_in_updateStatus($id,$status){
		$this->db->trans_start();
		$this->db->where('id', $id);
		$this->db->update('check_in_details', $status); 
		$this->db->trans_complete();
		return $this->db->trans_status();
	}
	public function check_in_delete($id){
		$this->db->trans_start();
		$this->db->where('id', $id);
		$this->db->delete('check_in_details'); 
		$this->db->trans_complete();
		return $this->db->trans_status();	
	}
	...
	public function isTitleExists($data){
		$this->db->select('*');
		$this->db->where('airline_code',$data);
