...
function getRoomType()
{
	...
	function(data) {
		if(data != ''){
			...
			roomTypelist += '<select name="room_type" id="room_type" class="form-control room_type1">';
			...
			roomTypeCapacitylist += '<select name="capacity_type" id="capacity_type" class="form-control">';
...
$('#datepicker12').change(function(){
    ...
    <title>Add New Room Category| InnoGlobe</title>
  ...
  <form class="validate form-horizontal" action="<?php echo WEB_URL;?>hotelcrs/add_rate_plan/admin" name="form1" method="post" enctype="multipart/form-data">
                             ...
                             <div class='form-group'>
                          	<label class='control-label col-sm-3'  for='validation_current'>Hotel Name: </label>
                             	...
                             	<select name="hotel_name" id="hotel_name"   class='form-control'  data-rule-required='true'  onChange="getRoomType();">
												...
												{
												?>
												<option  value="<?php  echo $all_hotels[$iii]->sup_hotel_id; ?>" <?php if(isset($result->sup_apart_list_id) && $result->sup_apart_list_id == $all_hotels[$iii]->sup_hotel_id) { echo "selected='selected'"; } ?>><?php echo $all_hotels[$iii]->hotel_name.' - '.$all_hotels[$iii]->city;  ?></option>
												<?php	
												}
												?>
											</select> 
                        ...
                        <div class='form-group'>
               			   ...
               			   <div  id="roomTypes"  class='col-sm-4 controls' >
                             
                             
                         
 
 	                        <select  name="room_type" id="room_type"    class='form-control room'  data-rule-required='true'  disabled=""  >
 	 										<option>Select Room Category</option>
											<?php foreach($result as $res) { ?>
										 	<option value="<?php echo $res->room_type; ?>" ><?php echo $res->room_type; ?></option>
												<?php }  ?> 
											 	</select>
								  	<span style="color:#FF0000;"> <?php echo form_error('room_type');?></span>
                             ...
                             <div class="" id="roomTypeCapacity">
                             
                             <select name="capacity_type" id="capacity_type" class="form-control"   disabled="">
                             	<option>Select Room Type</option>
                             <?php foreach($result as $rs1){  ?>
                             <option value="<?php $result->room_category; ?>" ><?php $rs1->room_category; ?></option>
                             <?php } ?>
                             </select>
                             
										<!--	<?php if(isset($result->room_category) && $result->room_category!=''){ 
												echo '<select name="capacity_type" id="capacity_type" class="form-control" >
												<option value="'.$result->room_category.'" >'.$result->room_category.'</option>
												</select>'; } else { ?>
												...
												<?php	 } ?>
												-->
												<span style="color:#FF0000;"> <?php echo form_error('capacity_type');?></span>
										</div>  
                                   </div>
                       				 </div>
                         ...
                         <input type="file" name="main_image" id="filepath" title='Search for a image to add' class='form-control'>
                 	...
                 	<input type="file" name="image3" id="filepath3" title='Search for a image to add' class='form-control' >
                       				 ...
                       				 <div class='form-group'>
                          	<label class='control-label col-sm-3'  for='validation_current'>Policy: </label>
                          	<div class='col-sm-4 controls'>
                      
                          
											<?php 
											$policy = $this->Hotelcrs_model->get_child_policy($this->uri->segment(4));
											   if(isset($policy->child_policy)) { $poli = $policy->child_policy; }else{ $poli = ''; }
											   ?>
											<textarea name="child_policy" class='form-control'  data-rule-required='true' id="child_policy" cols="70" rows="4"><?php echo $poli;?></textarea>												
                       				   ...
                       				   <div class='form-group'>
                          <label class='control-label col-sm-3 col-sm-3' for='validation_name'>Cancellation Policy:</label>
                          <div class='col-sm-2 controls'>
                            Nights &nbsp &nbsp &nbsp<input class='form-control' data-rule-minlength='2'   name="cancel_policy_nights[]" id="cancel_policy_nights[]"  placeholder='Nights' type='text'>
                          </div>
                          <div class='col-sm-3 controls'> 
                             Charge &nbsp &nbsp &nbsp <input class='form-control' data-rule-minlength='2'   id="cancel_policy_percent[]" name="cancel_policy_percent[]"  placeholder='Cancellation Charge' type='text'>
                          </div>
                          <div class='col-sm-3 controls'>
                            Or &nbsp &nbsp &nbsp <input class='form-control' data-rule-minlength='2'   id="cancel_policy_charge[]" name="cancel_policy_charge[]" placeholder='' type='text'>
                          </div>
                        </div>
                        
                           <div class='form-group'>
                          <label class='control-label col-sm-3 col-sm-3' for='validation_name'> </label>
                          <div class='col-sm-4 controls'>
                            Per Night Charge 	Cancellation allowed before days: &nbsp &nbsp &nbsp<input class='form-control'    id='validation_saluation' name='sal' placeholder='' type='text'>
                          </div>
                       	...
                       	<!--			 <div class='form-group'>
                              ...
                              <table summary="" width="600" >
										...
										<td width="">Nights:</td>
										 <td width="" align="left"> 
                                         <input size="15" type="text"  class='form-control' name="cancel_policy_nights[]" id="cancel_policy_nights[]" /></td>
										 <td>Charge</td> 
										 <td><input size="15" type="text" id="cancel_policy_percent[]" name="cancel_policy_percent[]"  class='form-control' />%&nbsp;</td> 
										 <td>or</td><td><input type="text"   id="cancel_policy_charge[]" name="cancel_policy_charge[]"  class='form-control' /></td> 
										</tr> 
										
										<tr>
										 <td colspan="3"> 
										Per Night Charge&nbsp;	Cancellation allowed before days: </td> 
										<td><input type="text"  class='form-control' name="cancellation_before_days" value="" /></td>
										  </tr>
										</table>      		 
										</div> 
                           ...
                           <div class='input-group'>
                       			<input type="hidden" id="apartfec_val" name="apartfec_val" value=""/>
                       				  ...
                       				  <div class='form-group'>
                          <label class='control-label col-sm-3 col-sm-3' for='validation_name'> Room Available Dates:</label>
                          ...
                          From : &nbsp <input name="sd[]" id="datepicker2" type="text" class="b2b-txtbox"  /> To : &nbsp<span id="out"><input name="ed[]" id="datepicker3" type="text" class="b2b-txtbox" />
                            </div>
                    	...
                    	<?php
									            ...
									            $next_dte = date("d-m-Y",strtotime("+8 days"));
									        ?>
									  <table summary="" width="800" > <tr><td>  
									        From :<input name="sd[]" id="datepicker2" type="text" class="b2b-txtbox"  />
									        To:<span id="out"><input name="ed[]" id="datepicker3" type="text" class="b2b-txtbox" /></span>
									         </td></tr></table>   
                     		...
                     		<div class='col-sm-6'>
                           <div class='input-group'>
                    	<input name="day" id="dayy" type="checkbox" value="Mon" style="margin-right:5px;"/>Mon <br>
								            <input name="day" id="dayy" type="checkbox" value="Tue" style="margin-right:5px;"/>Tue <br>
								            <input name="day" id="dayy" type="checkbox" value="Wed" style="margin-right:5px;"/>Wed <br>
								            <input name="day" id="dayy" type="checkbox" value="Thu" style="margin-right:5px;"/>Thu<br />
								            <input name="day" id="dayy" type="checkbox" value="Fri" style="margin-right:5px;"/>Fri<br>
								            <input name="day" id="dayy" type="checkbox" value="Sat" style="margin-right:5px;"/>Sat <br>
    ...
    <table width="100%"  align="left"   class="display tableStatic">
    ...
    <td width="12%" style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;">
    	<input name="price[]" id="price" type="text" class='form-control'  size="5"/></td>
    ...
    <td width="11%" style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;">
    	<input name="avail[]" id="avail" type="text" class='form-control'   size="5" onBlur="checkMaxRoom(this.value)"/></td>
    <td width="12%" style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;">
    	<input name="adult[]" id="adult" type="text" class='form-control'    size="5"/></td>
    <td width="10%" style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;">
    	<input name="child[]" id="child" type="text" class='form-control'  size="5"/></td>
    <!-- <td style="border-right:solid 1px #deab6f; text-align:center; padding-left:0px;">
...
<tr>
	...
	<input type="submit" value="submit" class="btn btn-primary">
	 ...
	 <a href="<?php echo WEB_URL; ?>hotelcrs/index">
...
$(document).ready(function(){
$("#datepicker3").live("change",function(){
	...
	var capacity=$("#capacity_type").val();
	
	var sd = $("#datepicker2").val();
	
	var ed = $("#datepicker3").val(); 
	
		 alert(hotel); alert(room); alert(sd);  alert(ed); 
	var api=$(".api").val();
 ...
 $("#msg2").css({"color":"red"});
	   ...
	   $.ajax({
		type: "POST",
		url: "<?php echo WEB_URL; ?>class/method",
	 	data: { age :agent, con:country, api:api, cat:hotel},
	 	dataType: 'json',
		success: function (msg){
			if(msg.status == '0'){
				$("#msg2").show();
				$("#msg2").html('Already This Hotel Room has been added to this Date');
		   	$('#tempStatus2').val('false');
			}else{
				$('#tempStatus2').val('true');
				$("#msg2").hide();
				$("#msg2").html('');
			} 
...
function validateDup2(){
	var status = $('#tempStatus2').val();
	if(status == 'false'){
		return false;
	}else{
		return true;
...
$( "#hotel_name" ).change(function() {
 var da=$( "#hotel_name" ).val();
  if(da==''){
  	$('#room_type, #capacity_type').attr('disabled','disabled');
...
$( "#room_type" ).change(function() {
 var da1=$( "#room_type" ).val();
 alert(da1)
  if(da1==''){
  	$('#capacity_type').attr('disabled','disabled');
