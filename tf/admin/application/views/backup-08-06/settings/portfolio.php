...
<html>
<head>
    <title>B2B Agent Management | InnoGlobe</title>
                ...
                <div class='col-sm-12'>
                  <div class='box bordered-box orange-border' style='margin-bottom:0;'>
                    <div class='box-header blue-background'>
                      ...
                      <div class='actions'>
                       <a href="<?php echo WEB_URL; ?>home_settings/portfolioForm"> <button class='btn' style='margin-bottom:5px'  ><i class='icon-male'></i> Add New Portfolio</button></a>
                       ...
                       <a class="btn box-collapse btn-xs btn-link" href="#"><i></i></a>
                          ...
                          <table class='data-table-column-filter table table-bordered table-striped' style='margin-bottom:0;'>
                            ...
                            <tbody>
                              ...
                              <?php  if(!empty($portfolio)){ $c=1;foreach($portfolio as $port){?>
                              <tr>
                                <td><?=$c;?></td>
                                <td><a data-lightbox='flatty' href='<?php echo $port->image;?>'>
                                  <img width="50" height="50" title="Image title" alt="" src="<?php echo $port->image;?>"></a></td>
                                <td><?=$port->link;?></td>
                                 <td>
                                 ...
                                 <select onchange="activate(this.value);">
                                   <?php if($port->status == '1'){?>
                                    <option value="<?php echo WEB_URL; ?>home_settings/updatePortfolioStatus/<?php echo $port->id; ?>/1" selected>Activate</option>
                                    <option value="<?php echo WEB_URL; ?>home_settings/updatePortfolioStatus/<?php echo $port->id; ?>/0">De-activate</option>
                                    <?php }else{?>
                                    <option value="<?php echo WEB_URL; ?>home_settings/updatePortfolioStatus/<?php echo $port->id; ?>/1">Activate</option>
                                    <option value="<?php echo WEB_URL; ?>home_settings/updatePortfolioStatus/<?php echo $port->id; ?>/0" selected>De-activate</option>
                                  </select>
                                  <?php }?>
                                </td>
                                <td>
			 	 <div class='text-right'>
				<a class='btn btn-info btn-xs has-tooltip' data-placement='top' title='Edit User' href='<?php echo WEB_URL; ?>home_settings/getOnePortfolio/<?=$port->id;?>'>
                                      <i class='icon-edit'></i>
                                    </a>
				 <a class='btn btn-danger btn-xs has-tooltip' data-placement='top' title='Delete' href='<?php echo WEB_URL; ?>home_settings/deleteOnePortfolio/<?=$port->id;?>'>
