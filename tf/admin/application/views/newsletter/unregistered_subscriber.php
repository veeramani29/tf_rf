...
<html>
<head>
  <title>News letter subscribers | <?php echo PROJECT_NAME;?></title>
                  ...
                  <div class='responsive-table'>
                    <div class='scrollable-area'>
                      <form method="post" id="myform" action="<?php echo base_url(); ?>newsletter/export_all_subscribers">
                        <?php if (!empty($unreg_sub_list)) { ?>
                          <?php foreach ($unreg_sub_list as $users) { ?>
                        
                          <input type="hidden" name="cid[]" value="<?php echo $users->email_id; ?>">
                        ...
                        <table class='data-table-column-filter table table-bordered table-striped' style='margin-bottom:0;'>
                          ...
                          <tbody>
                            <?php if (!empty($unreg_sub_list)) {
                              $c = 1;
                              foreach ($unreg_sub_list as $users) { ?>
                              <tr>
                                <td><?= $c; ?></td>
                                <td><?php echo $users->email_id; ?></td>
                                <td>
                                  <a class="btn btn-danger btn-xs has-tooltip" onclick="return confirm('Are you sure?')" data-placement="top" title="" href="<?php  echo WEB_URL.'newsletter/unregistered_deleteSubscriber/'.$users->id; ?>" data-original-title="Delete subscriber">
