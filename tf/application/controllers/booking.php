        ...
        foreach ($cids as $key => $cid) {
			...
			}else if($module == 'FLIGHT')
            {
               ...
               if(isset($checkout_form['middle_name'.$cid]) && $checkout_form['middle_name'.$cid]!=''){
                ...
                $db_array["wheel_ssr"]=array(); $db_array["meals_ssr"]=array();$db_array["ffn_Car"]=array(); $db_array["ffn_num"]=array();
                if(isset($checkout_form["pfirst_name$cid"])){
                    ...
                    $wheel_ssr_Arr=$checkout_form["pwheelchair$cid"];
                    $meals_ssr_Arr=$checkout_form["pmeals$cid"];
                    $ffnCarr_Arr=$checkout_form["pffnCarr$cid"];
                    $ffn_Arr=$checkout_form["pffn$cid"];
                    ...
                    foreach($dob_Arr as $key=>$value){
                     ...
                     foreach($wheel_ssr_Arr as $key=>$value){
                        $db_array["wheel_ssr"][]=array("$key"=>$value);
                    }
                     foreach($meals_ssr_Arr as $key=>$value){
                        $db_array["meals_ssr"][]=array("$key"=>$value);
                    }


                     foreach($ffnCarr_Arr as $key=>$value){
                        $db_array["ffn_Car"][]=array("$key"=>$value);
                    }

                     foreach($ffn_Arr as $key=>$value){
                        $db_array["ffn_num"][]=array("$key"=>$value);
                    }
			    }
                if($checkout_form['insurance_type']!='No Insurance' && $checkout_form['insurance_type']!='Cancellation Insurance'){
                	if($checkout_form['insurance_type']=='Travel Insurance'){
                $insurance_applied_no_person=$checkout_form['insurance_applied_t_person'];
                	}else{
                $insurance_applied_no_person=$checkout_form['insurance_applied_tc_person'];
                	}

                }else{
                $insurance_applied_no_person=1;
                }
				...
				$booking_flight = array(
                    ...
                    'GUEST_EMAIL' => $checkout_form['email'],
                    'GUEST_PHONE' => $country_code1.$checkout_form['mobile'] ,
					'GUEST_MIDDLENAME' => $checkout_form['middle_name'],
                    ...
                    'insurance_type' => $checkout_form['insurance_type'],
                    'insurance_amount' =>($checkout_form['insurance_amount']*$insurance_applied_no_person),
                    'insurance_applied_no_person' =>$insurance_applied_no_person,
					'USER_TYPE' => $user_type,
                    'USER_ID' => $user_id,
                ...
                if(isset($_SESSION['RedeemStatus']) && $_SESSION['RedeemStatus'] == 'Yes' && $FinalTotalPrice >= 0){
                    $Redeempoints = $this->account_model->ReturnRedeemPoints($TOTAL);
                    $FinalTotalPrice=$Redeempoints*0.0075;
                    $FinalTotalPrice = ($TOTAL+$cart_flight_data->HiddenMarkupDiscount) - $FinalTotalPrice;
                    $PayedAsAmount = $FinalTotalPrice;
                }else{
                    $Redeempoints = 0;
                    $FinalTotalPrice=0;
                    $TotalPointsDebited = 0;
                    $PayedAsAmount = ($TOTAL+$cart_flight_data->HiddenMarkupDiscount);
                }
                ...
                $booking = array(
                    ...
                    'parent_pnr' => $parent_pnr,
                    'user_type' => $user_type,
                    'amount' => ($TOTAL+$cart_flight_data->HiddenMarkupDiscount),
                    'NWERPointsDebited' => $Redeempoints,
                ...
                $total_days=($cart_flight_data->DepartureTime)-($cart_flight_data->ArrivalTime);
                $period = abs(floor($total_days/(60*60*24)));

                $insurance_details = array(
                'module' => 'FLIGHT',
                'booking_ref_id' => $booking_flight_id,
                'pnr_no' => $pnr_no,
                'full_name' => $guest_fname.$guest_lname,
                'email' => ($checkout_form['email']),
                'insurance_type' => $checkout_form['insurance_type'],
                'insurance_amount' =>($checkout_form['insurance_amount']*$insurance_applied_no_person),
                'insurance_tax' => ($checkout_form['insurance_tax']*$insurance_applied_no_person),
                'insurance_applied_no_person' => $insurance_applied_no_person,
				'period' => $period,
                'amount' => ($TOTAL+$cart_flight_data->HiddenMarkupDiscount),
                'booking_status' => 'PROCESS'
                );
                $this->flight_model->Insurance_Details($insurance_details);
                // Inserting insurance details
                ...
                if(isset($_SESSION['RedeemStatus']) && $_SESSION['RedeemStatus'] == 'Yes' && isset($_SESSION['RedeemPoints']) && $_SESSION['RedeemPoints'] > 0){
                    $InsertLoyaltyPoints = array(
                        ...
                        'UserType' => $user_type,
                        'Points' => $Redeempoints,
                         'currPoints' => $cart_flight_data->BasePrice,
                        'BookingId' => $pnr_no,
                    ...
                    $InsertLoyaltyPoints = array(
                        'UserID' => $user_id,
                        'UserType' => $user_type,
                        'Points' => ($cart_flight_data->BasePrice),
                        'BookingId' => $pnr_no,
				...
				if($book_check=='ok'){
					...
					if($module == 'FLIGHT')
                    {
						...
						if($count == 1)
                        {
							...
							'AirReservationLocatorCode' => $ProviderLocatorCode,
							'api_status' => $Status,
					...
					if($module == 'HOTEL')
					{
						...
						{							
							...
							{
    							$BookingStatus = 'CONFIRMED';
    							$LocatorCode = '';$ProviderLocatorCode = '';
                                $AirReservationLocatorCode = '';
                                $HotelReservationConfirmationCode = '';
                                $BookingTravelerRef = '';
    							$Status = 'CONFIRMED';
    							$Remarks='';
    							$books_req =  'CRS';
