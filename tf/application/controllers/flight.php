...
class Flight extends CI_Controller {
	...
	public function search_flight(){
		...
		$myMarkup = $MyMarkup['markup'];
		 //echo ($myMarkup);die;
		if($req_data->type == 'M'){
			$results = $this->formatMultiResponse($LowFareSearchRes, $aMarkup, $myMarkup);
			
		}else{
			$results = $this->formatResponse($LowFareSearchRes, $aMarkup, $myMarkup);
		}
	
		$data['flights'] = $results;
		$data['req'] = $req_data;
		$data['request']  = $req_data;
		$this->load->view('flight_new/results1', $data);
	}
	...
	$this->session->set_userdata($this->url);
		...
		$request = $this->input->get();
		$request['search_data_expiry']=date("H:i", strtotime('5 minutes', time()));
		//echo '<pre>';print_r($request);exit;

		$this->session->set_userdata('search_data',$request);

		
		if($request['trip_type'] != 'multicity'){
			if(strpos($request['from'], " (All Airports)")){
				$from_origin=str_replace(" (All Airports)","",$request['from']);
			}else{
				$from_origin=$request['from'];
			}
		
			if(strpos($request['to'], " (All Airports)")){
				$to_destination=str_replace(" (All Airports)","",$request['to']);
			}else{
				$to_destination=$request['to'];
			}
			$origin = '&origin='.substr(chop(substr($from_origin, -5), ')'), -3);
			$destination = '&destination='.substr(chop(substr($to_destination, -5), ')'), -3);
			$depart_date = '&depart_date='.str_replace('/', '-', $request['depature']);
			if(isset($request['days']) && $request['days']==1){
			$extradays = '&days='.$request['days'];
			}else{
			$extradays = '&days';
			}
			if(isset($request['disc_price']) && $request['disc_price']!=''){
			$disc_price = '&disc_price='.$request['disc_price'];
			}else{
			$disc_price = '';
			}
			if(isset($request['Carriers']) && $request['Carriers']!=''){
			$Carriers = '&Carriers='.$request['Carriers'];
			}else{
			$Carriers = '';
			}
		...
		$cls=($request['class']!=null)?$request['class']:'';
		 $class = '&class='.$cls;
		if($request['trip_type'] == 'oneway'){
			$type = 'type=O';
			
			$query = $type.$origin.$destination.$depart_date.$adult.$child.$infant.$class.$extradays.$disc_price.$Carriers.$provider.$method;
		}else if($request['trip_type'] == 'circle'){
			$type = 'type=R';
			
			$return_date = '&return_date='.str_replace('/', '-', $request['return']);
			$query = $type.$origin.$destination.$depart_date.$return_date.$adult.$child.$infant.$class.$extradays.$disc_price.$Carriers.$provider.$method;
		}else if($request['trip_type'] == 'multicity'){
			$type = 'type=M';
			//$return_date = '&return_date='.$request['return'];
			$multi = json_decode(json_encode($this->input->get()));
			//echo '<pre>';print_r($multi);exit;
	...
	public function GetResults($request=''){
		...
		$LowFareSearchRes = LowFareSearchReq($request);
		$aMarkup = $this->account_model->get_markup('FLIGHT'); //get markup
		  $aMarkup = $aMarkup['markup'];
		...
		$data['flex_air_segment_date']=$flex_air_segment_date;
			...
			if($request->type != 'M'){
				if($request->days == 1){
				$data['graph_flights'] = $results;
				$this->load->view('flight/table_results', $data);
			}
			if($request->type == 'O'){
...
//if($c_user_id!='' && $c_user_type!='' && $data['Booking']->user_id == $c_user_id && $data['Booking']->user_type == $c_user_type)
	...
	public function AddToCart(){
		...
		$SAdults = $request->ADT;
		$SChildren = $request->CHD;
			// Added By Mukesh
			$MCalcCarriers = array();
			$WholeMarkups = array();
			$HiddenMarkupsArray = array();
			$HiddenFeeAmount = 0;
			$VisibleMarkupsArray= array();
			$AllSegments= array();
	    ...
	    foreach ($MCalcCarriers as $MCalcCarrier) {
	      ...
	      foreach ($WholeMCalcCarrier as $WholeM) {
	        $WholeMarkups[] = array($WholeM->Type, $WholeM->AmountType, ($WholeM->AmountType == 'Currency') ? $WholeM->Currency : $WholeM->Percent, $WholeM->MDId, $WholeM->HiddenStatus, $WholeM->Description,$WholeM->Per,$WholeM->MaxAmount);
          	//break;
	      }
	      break;
	    }
	    ...
	    foreach ($WholeMarkups as $WholeMarkup) {
			...
			$MrkupFeeHStatus = $WholeMarkup[4];

			if($MrkupFeeHStatus == 'Y'){

			$MrkupFeeType = $WholeMarkup[1];
			$WholeMarkupFeeAmount = $WholeMarkup[2];
			$MrkupFeeDesc = $WholeMarkup[5];
			$WholeMarkupPer = $WholeMarkup[6];
			$WholeMarkupMaxAmount = $WholeMarkup[7];
			...
			{
			//echo '<pre>';print_r($WholeMarkup);;
			$MrkupFeeType = $WholeMarkup[1];
			$WholeMarkupFeeAmount = $WholeMarkup[2];
			$MrkupFeeDesc = $WholeMarkup[5];
			$WholeMarkupPer = $WholeMarkup[6];
			$WholeMarkupMaxAmount = $WholeMarkup[7];
			$org_ticketcharge_cost=$WholeMarkupFeeAmount;
		...
		($MrkupFeeType == 'Currency') ? $MarkupFeeAmount = $WholeMarkupFeeAmount : $MarkupFeeAmount = (($FlightPriceWithoutMarkup*$WholeMarkupFeeAmount)/100);
			// Check Markup Maxamount
			if($WholeMarkupFeeAmount > $WholeMarkupMaxAmount){
				$WholeMarkupFeeAmount = $WholeMarkupMaxAmount;
			}

		($WholeMarkup[0] == 'Addition') ? $HiddenFeeAmount += $WholeMarkupFeeAmount : $HiddenFeeAmount -= $WholeMarkupFeeAmount;
		($WholeMarkup[0] == 'Addition') ? $VisibleFeeType = '+' : $VisibleFeeType = '-';
		$HiddenMarkupsArray[] = array($MrkupFeeDesc,$VisibleFeeType,$HiddenFeeAmount,$WholeMarkupPer,$org_ticketcharge_cost);
			}
	    }
	   	
	     $TotalMarkupFeeAmount = $this->account_model->currency_convertor($TotalMarkupFeeAmount); // Final Markup/Discounted Value based on $FlightPriceWithoutMarkup
		...
		if(isset($AirPriceRes->Body->AirPriceRsp->AirPriceResult)){
	    	...
	    	$TotalPrice = $this->account_model->PercentageToAmount($TotalPrice,$myMarkup) + $TotalMarkupFeeAmount;
	    	...
	    	$cart_flight = array(
				...
				'SITE_CURR' => CURR,
				'MyMarkup' => $Markup,
				'HiddenMarkupDiscountArr' => json_encode($HiddenMarkupsArray),
				'HiddenMarkupDiscount' => $HiddenFeeAmount,
				'VissibleOthersMarkup' => $TotalMarkupFeeAmount,
	        ...
	        if(count($cart_data) > 0){
	            $data['isCart'] = true;
	             $data['is_xml_msg'] = $AirPriceErrorRes_msg;
	        }else{
	            $data['isCart'] = false;
	            $data['is_xml_msg'] = $AirPriceErrorRes_msg;
	        }
	...
	function formatResponse($LowFareSearchReq_Res, $aMarkup, $myMarkup,$method){
	   ...
	   // echo '<pre>';print_r($LowFareSearchRes);die;
		...
		else
		{
			...
			if($method=="asynch"){
			$Search_method="air:LowFareSearchsynchRsp";
		...
		$more_results['SearchId']=$SearchId;
			  ...
			  // echo '<pre>';print_r($AirPricingSolutions);exit;
			    $i=0;
		...
		$AirPricingSolution=$AirPricingSolutions;
	    	...
	    	$fs[$i]['RedEye'] = isset($AirPricingInfo['air:ChangePenalty']['air:Amount'])?$AirPricingInfo['air:ChangePenalty']['air:Amount']:'';
                    $fs[$i]['ShortLayOver'] = isset($AirPricingInfo['air:ChangePenalty']['air:Amount'])?$AirPricingInfo['air:ChangePenalty']['air:Amount']:'';
                    $fs[$i]['LongLayOver'] = isset($AirPricingInfo['air:ChangePenalty']['air:Amount'])?$AirPricingInfo['air:ChangePenalty']['air:Amount']:'';

	    	if($Refundable == 'true'){
				$FareType = 'Refundable';
				$fs[$i]['ChangePenalty'] = isset($AirPricingInfo['air:ChangePenalty']['air:Amount'])?$AirPricingInfo['air:ChangePenalty']['air:Amount']:'';
                 $fs[$i]['CancelPenalty'] = isset($AirPricingInfo['air:CancelPenalty']['air:Amount'])?$AirPricingInfo['air:CancelPenalty']['air:Amount']:'';
	    	}else{
	    	...
	    	if(isset($AirPricingSolution['air:Journey'][0])){
	    		...
	    		foreach ($Journies as $key => $Journey) {
					...
					if(isset($AirSegmentRefs[0])){
						foreach ($AirSegmentRefs as $jk => $AirSegmentRef) {
							...
							foreach ($AirSegments as $key => $AirSegmentt) {
								...
								if($AirSegment_Key == $AirSegmentRef_Key){
									...
									$fs[$i][$mode]['segments'][$jk]['Distance'] = $Distance = $AirSegment_Attr['Distance'];
									$fs[$i][$mode]['segments'][$jk]['ETicketability'] = $ETicketability = $AirSegment_Attr['ETicketability'];
							...
							$fs[$i][$mode]['segments'][$jk]['AvailabilityDisplayType'] =  $AirSegment_Attr['AvailabilityDisplayType'];
									...
									$fs[$i][$mode]['segments'][$jk]['PolledAvailabilityOption'] = $PolledAvailabilityOption = $AirSegment_Attr['PolledAvailabilityOption'];
									$fs[$i][$mode]['segments'][$jk]['OptionalServicesIndicator'] = $OptionalServicesIndicator = $AirSegment_Attr['OptionalServicesIndicator'];
									$AvailabilitySource = '';
						...
						$Farerulesref_Key=array();$Farerulesref_Provider=array();$Farerulesref_content=array(); $BaggageAllowance_arr = array();
	    					...
	    					$faredetails=$FareInfoses['air:FareRuleKey'];
								 ...
								 if($FareInfoRef==$FareInfoRef_key){
									...
									$fs[$i][$mode]['segments'][$jk]['Farerulesref_content'] =  $farecontent;
									 ...
									 // Getting Baggage Allownce Start
                                                if (isset($FareInfoses['air:BaggageAllowance'])) {
                                                    $BaggageAllowancedetails = $FareInfoses['air:BaggageAllowance'];
                                                    if (isset($BaggageAllowancedetails['air:MaxWeight'])) {
                                                        $maxweight = $BaggageAllowancedetails['air:MaxWeight'];
                                                        if (isset($maxweight['@attributes'])) {
                                                            $maxweightattributes = $maxweight['@attributes'];
                                                            $BaggageAllowance = $maxweightattributes['Value'] . " " . $maxweightattributes['Unit'];
                                                        } elseif (isset($BaggageAllowancedetails['air:NumberOfPieces'])) {
                                                $BaggageAllowance = "NumberOfPieces " . $BaggageAllowancedetails['air:NumberOfPieces'];
                                            }else {
                                                            $BaggageAllowance = "Yes";
                                                        }
                                                    } elseif (isset($BaggageAllowancedetails['air:NumberOfPieces'])) {
                                                        $BaggageAllowance = "NumberOfPieces " . $BaggageAllowancedetails['air:NumberOfPieces'];
                                                    }
                                                    $BaggageAllowance_arr[] = $BaggageAllowance;
                                                    $fs[$i][$mode]['segments'][$jk]['BaggageAllowance'] = $BaggageAllowance;
                                                }// Getting Baggage Allownce end
							...
							$faredetails=$FareInfos['air:FareRuleKey'];
								 ...
								 //echo '<pre>';print_r($AirPricingSolution);die;
								   $FareInfoRef_key =  $FareInfos_Attr['FareInfoRef'];	
									...
									$fs[$i]['BaggageAllowance'][] = $BaggageAllowance_arr;
						...
						foreach ($AirSegment as $key => $AirSegmentt) {
							...
							if($AirSegment_Key == $AirSegmentRef_Key){
								...
								$fs[$i][$mode]['segments'][0]['FlightTime'] = $FlightTime = $AirSegment_Attr['FlightTime'];
								if(isset($AirSegment_Attr['APISRequirementsRef'])){
								 ...
								 if($FareInfoRef==$FareInfoRef_key){
									...
									// Getting Baggage Allownce Start
                                            if (isset($FareInfoses['air:BaggageAllowance'])) {
                                                ...
                                                $BaggageAllowance_arr[] = $BaggageAllowance;
                                                $fs[$i][$mode]['segments'][0]['BaggageAllowance'] = $BaggageAllowance;
                                            }// Getting Baggage Allownce end
					...
					foreach ($AirSegmentRefs as $jk => $AirSegmentRef) {
						...
						foreach ($AirSegments as $key => $AirSegmentt) {
							...
							if($AirSegment_Key == $AirSegmentRef_Key){
								...
								$fs[$i]['segments'][$jk]['Distance'] = $Distance = $AirSegment_Attr['Distance'];
								$fs[$i]['segments'][$jk]['ETicketability'] = $ETicketability = $AirSegment_Attr['ETicketability'];
							...
							$fs[$i]['segments'][$jk]['AvailabilityDisplayType'] =  $AirSegment_Attr['AvailabilityDisplayType'];
								...
								$fs[$i]['segments'][$jk]['PolledAvailabilityOption'] = $PolledAvailabilityOption = isset($AirSegment_Attr['PolledAvailabilityOption'])?$AirSegment_Attr['PolledAvailabilityOption']:'';
								$fs[$i]['segments'][$jk]['OptionalServicesIndicator'] = $OptionalServicesIndicator = $AirSegment_Attr['OptionalServicesIndicator'];
								$AvailabilitySource = '';
								if(isset($AirSegment_Attr['AvailabilitySource'])){	
									$AvailabilitySource = $AirSegment_Attr['AvailabilitySource'];
								}
								 ...
								 if($FareInfoRef==$FareInfoRef_key){
									...
									// Getting Baggage Allownce Start
                                            if (isset($FareInfoses['air:BaggageAllowance'])) {
                                                ...
                                                $BaggageAllowance_arr[] = $BaggageAllowance;
                                                $fs[$i]['segments'][$jk]['BaggageAllowance'] = $BaggageAllowance;
                                            }// Getting Baggage Allownce End
								 ...
								 if($FareInfoRef==$FareInfoRef_key){
									...
									// Getting Baggage Allownce Start
                                        if (isset($FareInfoses['air:BaggageAllowance'])) {
                                            ...
                                            $BaggageAllowance_arr[] = $BaggageAllowance;
                                            $fs[$i]['segments'][0]['BaggageAllowance'] = $BaggageAllowance;
                                        }// Getting Baggage Allownce End
...
function formatMultiResponse($LowFareSearchReq_Res, $aMarkup, $myMarkup,$method)
{
						...
						foreach ($AirSegmentRefs as $jk => $AirSegmentRef) {
							...
							foreach ($AirSegments as $key => $AirSegmentt) {
								...
								if($AirSegment_Key == $AirSegmentRef_Key){
									...
									$fs[$i]['segments'][$jj][$num_seg]['ParticipantLevel'] = $ParticipantLevel = isset($AirSegment_Attr['ParticipantLevel'])?$AirSegment_Attr['ParticipantLevel']:'';
									if(isset($AirSegment_Attr['APISRequirementsRef'])){
							...
							$fs[$i]['segments'][$jj][$num_seg]['AvailabilityDisplayType'] =  $AirSegment_Attr['AvailabilityDisplayType'];
									...
									$fs[$i]['segments'][$jj][$num_seg]['PolledAvailabilityOption'] = $PolledAvailabilityOption = isset($AirSegment_Attr['PolledAvailabilityOption'])?$AirSegment_Attr['PolledAvailabilityOption']:'';
								 ...
								 if($FareInfoRef==$FareInfoRef_key){
									...
									// Getting Baggage Allownce Start
                                        if (isset($FareInfoses['air:BaggageAllowance'])) {
                                            ...
                                            $BaggageAllowance_arr[] = $BaggageAllowance;
                                            $fs[$i]['segments'][$jj][$num_seg]['BaggageAllowance'] = $BaggageAllowance;
                                        }// Getting Baggage Allownce End
	    					...
	    					$faredetails=$FareInfoses['air:FareRuleKey'];
								 ...
								 if($FareInfoRef==$FareInfoRef_key){
                                    ...
                                    if (isset($FareInfoses['air:BaggageAllowance'])) {
                                        ...
                                        $BaggageAllowance_arr[] = $BaggageAllowance;
                                        $fs[$i]['segments'][$jj]['BaggageAllowance'] = $BaggageAllowance;
                                    }// Getting Baggage Allownce End
	...
	public function cancel($pnr_no){
        ...
        if($count == 1) {
            ...
            $xml_log = array(
                'Api' => 'UAPI',
                'XML_Type' => 'FLIGHT - CANCELLATION -UniversalRecordCancelRsp ERROR',
  ...
  public function fareRule(){
		...
		$fare_message = <<<EOM
			<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
				<soapenv:Header/>
				<soapenv:Body>
					<ns2:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v33_0" xmlns:ns2="http://www.travelport.com/schema/air_v33_0" FareRuleType="long" TargetBranch="P2629804" AuthorizedBy="Test">
...
public function cancel_request(){
        ...
        $pnr_no = base64_decode(base64_decode($this->input->post('pnr')));
        $count = $this->booking_model->getBookingPnr($pnr_no)->num_rows();
        if($count == 1) {
            $b_data = $this->booking_model->getBookingPnr($pnr_no)->row();
            //echo '<pre>';print_r($b_data);die;
            if($b_data->booking_status == 'CONFIRMED'){
            	...
            	$leadpax=$b_data->leadpax;
            	$booking_no=$b_data->booking_no;
            	$AirReservationLocatorCode=$b_data->AirReservationLocatorCode;


				$MSG='<p><b>Hi Admin, following user sent request to '.$request_type.' their booking please check and do it.</b></p>'; 
				$MSG.='<p><b>Username :</b> '.$username.' </p>'; 
				$MSG.='<p><b>Lead Passanger :</b> '.$leadpax.' </p>'; 
				$MSG.='<p><b>Email :</b>  '.$email.' </p>'; 
				$MSG.='<p><b>Message</b> <br> '.$message.' </p>'; 
				$MSG.='<p><b>Confirmation No :</b> '.$pnr_no.' </p>'; 
				$MSG.='<p><b>Reservation Code :</b> '.$AirReservationLocatorCode.'  </p>'; 
				$MSG.='<p><b>Booking Code :</b> '.$booking_no.' </p>'; 
				$MSG.='<p><b>Booking Status :</b> '.$b_data->booking_status.' </p>'; 
				$MSG.='<p>Thanks <br> <b>Ticketfinder</b> </p>'; 
				...
				$data['email_access'] = $this->email_model->get_email_acess()->row();
				$Response = $this->email_model->sendmail_flight_cancel_request($data);



            	
            	//$parent_pnr=$b_data->parent_pnr;
				 $response = array('msg' => 1);
        		echo json_encode($response);
			   
            }
        }else{
            $response = array('msg' => 'Something went wrong pls try again');
        	echo json_encode($response);
        }

         }else{
            $response = array('msg' => 'please enter all the fields');
        	echo json_encode($response);
        }
    }

    public function veera(){

    	  $LowFareSearchRes = $this->xml_to_array->XmlToArray($a);
    	  $AdditionalCostList=array();$DiscountList=array();$CommentList=array();
		if(isset($LowFareSearchRes['ErrorList']['Error'])){
	    	$Booking['ErrorList']=$LowFareSearchRes['ErrorList']['Error'];
	    }
		
		

			if(isset($LowFareSearchRes['Purchase'])){
			$PurchaseDetails=$LowFareSearchRes['Purchase'];
			$ServiceListDetails=$LowFareSearchRes['Purchase']['ServiceList']['Service'];
			$HotelInfo=$ServiceListDetails['HotelInfo'];
			$AvailableRoom=$ServiceListDetails['AvailableRoom'];
		   ...
		   if(isset($PurchaseDetails['@attributes']['purchaseToken'])){
			...
			$CommentList[]=array($Comment_value['@attributes']=>$Comment_value['@content']);
			}
			$Booking['CommentList']=$CommentList;
				...
				if(isset($ServiceListDetails['DiscountList']['Price'])){
				$DiscountList_=$ServiceListDetails['DiscountList']['Price'];

				foreach ($DiscountList_ as $Discount => $Discount_value) {
				$DiscountList[]=array('Amount'=>$Discount_value['Amount'],'Description'=>$Discount_value['Description']);
				}
				$Booking['DiscountList']=$DiscountList;
				}
				
				if(isset($ServiceListDetails['AdditionalCostList']['AdditionalCost'])){
				$AdditionalCostList_=$ServiceListDetails['AdditionalCostList']['AdditionalCost'];

				foreach ($AdditionalCostList_ as $AdditionalCost => $AdditionalCost_value) {
				$AdditionalCostList[]=array('Amount'=>$AdditionalCost_value['Price']['Amount'],'type'=>$AdditionalCost_value['@attributes']['type']);
				}
				$Booking['AdditionalCostList']=$AdditionalCostList;
				}

				if(is_array($AvailableRoom)){
				

				foreach ($AvailableRoom as $AvailableRoom => $AvailableRoom_value) {
					$CancellationPolicies=$AvailableRoom_value['HotelRoom']['CancellationPolicies'];
				$CancellationPolicies[]=$CancellationPolicies;
				}
				$Booking['CancellationPolicies']=$CancellationPolicies;
				}
