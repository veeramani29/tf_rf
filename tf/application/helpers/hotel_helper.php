...
function get_hotel_list($data){
  ...
  if(!empty($data['rooms']))
  {
    ...
    foreach($data['rooms'] as $room){
      ...
      $SearchRoom .="<Adult>".$room['adult']."</Adult>";
      for($i=1;$i<=3;$i++){
        if(isset($room['childrenage'.$i]))
          $SearchRoom .="<Child".$i."Age>".$room['childrenage'.$i]."</Child".$i."Age>";
        else
          $SearchRoom .="<Child".$i."Age>0</Child".$i."Age>";
      }
      
      $SearchRoom .="<Quantity>1</Quantity></SearchRoom>";
    }
  }
  else
  {
    $SearchRoom = "<SearchRoom></SearchRoom>";
  }  
  $xml_request='<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
  <s:Body>
    <Search xmlns="http://tempuri.org/">
      <Header>
        <AgentId>12239</AgentId>
        <Password>leon2017reizen</Password>
        <Username>XML.INFO@DIYARREIZEN.NL</Username>
      </Header>
      <Search>
        <CheckIn>'.$data['check_in'].'</CheckIn>
        <CheckOut>'.$data['check_out'].'</CheckOut>
        <CityId>'.$data['city_code'].'</CityId>
        <CurrencyId>2</CurrencyId>
        <HotelId>0</HotelId>
        <PaxCountryId>2</PaxCountryId>
        <DefaultPhotoInclude>true</DefaultPhotoInclude>
        <GeoCoordinatesInclude>true</GeoCoordinatesInclude>
        <HotelAddressInclude>true</HotelAddressInclude>
        <HotelDescriptionInclude>true</HotelDescriptionInclude>
...
//echo $xml_request; die();
$respone = curl($xml_request,'http://tempuri.org/ILeon/Search','http://Leon.leonardotravel.com/Leon.svc/wsdl');
...
function get_hotel_details_results($data) {
  ...
  $xml_request='<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
   ...
   $return_data['request'] = $xml_request;
    ...
    $return_data['response'] = curl($xml_request,'http://tempuri.org/IStatic/HotelDetail','http://leont.leonardotravel.com/Static.svc/wsdl?wsdl');
...
function curl($xml_request,$actionurl,$curlurl) 
{
...
//leonardotravel REquests By Ganpat
//Makebook request
function makebook_request($data)
{
  $xml_request = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
    <s:Body>
      <MakeBook xmlns="http://tempuri.org/">
        <Header>
          <AgentId>12239</AgentId>
          <Username>XML.INFO@DIYARREIZEN.NL</Username>
          <Password>leon2017reizen</Password>
  ...
  $return_data['request'] = $xml_request;
    ...
    $return_data['response'] = curl($xml_request,'http://tempuri.org/ILeon/MakeBook','http://leon.leonardotravel.com/Leon.svc/wsdl');
...
//Function to get cancelation policy for hotel 
function get_policy_details($data)
{
  $xml_request = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
    <s:Body>
      <Policy xmlns="http://tempuri.org/">
        <Header>
          <AgentId>12239</AgentId>
          <Username>XML.INFO@DIYARREIZEN.NL</Username>
          <Password>leon2017reizen</Password>
  ...
  $return_data['request'] = $xml_request;
    ...
    $return_data['response'] = curl($xml_request,'http://tempuri.org/ILeon/Policy','http://leon.leonardotravel.com/Leon.svc/wsdl');
...
function get_confirmation($data)
{
  //print_r($data);
  $room_ids = json_decode(base64_decode($data['room_ids']));
  $rooms = json_decode(base64_decode($data['rooms']),true);
 // print_r($rooms);
  ...
  {
    $person_rooms .= '<PersonRoom RoomId="'.$room_ids[$r-1].'">
            <Passengers>';
            
            for($a=1;$a<=$rooms[$r]['adult'];$a++)
            {
              $person_rooms .= '<Passenger Adult="true" PrefixId="'.$data['gender-room'.$r.'adult'.$a].'" Name="'.$data['name-room'.$r.'adult'.$a].'" SurName="'.$data['surname-room'.$r.'adult'.$a].'" Age="'.$data['age-room'.$r.'adult'.$a].'"/>';
            }  
            for($c=1;$c<=$rooms[$r]['children'];$c++)
            {
              $person_rooms .= '<Passenger Adult="false" PrefixId="'.$data['gender-room'.$r.'child'.$c].'" Name="'.$data['name-room'.$r.'child'.$c].'" SurName="'.$data['surname-room'.$r.'child'.$c].'" Age="'.$data['age-room'.$r.'child'.$c].'"/>';
            }  
              
          $person_rooms .= '</Passengers>
          </PersonRoom>';
  }
  ...
  $xml_request = '<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
    <s:Body>
      <Confirm xmlns="http://tempuri.org/">
        ...
        <Confirm>
        <ConfirmCode>'.$data['confirm_token'].'</ConfirmCode>
        <AgentRef>Agent referange</AgentRef>
...
//echo $xml_request; die();
  $return_data['request'] = $xml_request;
    ...
    $return_data['response'] = curl($xml_request,'http://tempuri.org/ILeon/Confirm','http://leon.leonardotravel.com/Leon.svc/wsdl');
...
function cancel_booking($data)
{
  ...
  $return_data['request'] = $xml_request;
   $return_data['response'] = curl($xml_request,'http://tempuri.org/ILeon/Cancel','http://leon.leonardotravel.com/Leon.svc/wsdl');            
