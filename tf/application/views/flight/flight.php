...
class Flight extends CI_Controller {
	...
	public function search(){
		...
		$class = '&class='.isset($request['class'])?$request['class']:'';
	...
	public function GetResults($request=''){
		...
		$data['flex_air_segment_date']=$flex_air_segment_date;
			...
			if($request->type != 'M'){
				...
				$this->load->view('flight/graph_results', $data);
			}
			if($request->type == 'O'){
 ...
 echo json_encode(array('result' => $allview,'more_results'=>$more_results));
	...
	public function AirTicketingReq(){

		
 				$AirTicketingReq_Res = AirTicketingReq($a='');
            	 $AirTicketingRes = $AirTicketingReq_Res['AirTicketingRsp'];
            	$AirTicketingRes = $this->xml_to_array->XmlToArray($AirTicketingRes);
		 if(isset($AirTicketingRes['SOAP:Body']['universal:UniversalRecordCancelRsp'])){
			    	$AirTicketingRes = $AirTicketingRes['SOAP:Body']['universal:UniversalRecordCancelRsp']['universal:ProviderReservationStatus'];
			    	$allAttr = $AirTicketingRes['@attributes'];
			    	if($allAttr['Cancelled']){
			    	...
			    	$xml_log = array(
			    		'Api' => 'UAPI',
			    		'XML_Type' => 'FLIGHT - AirTicketing -UniversalRecordCancelRsp EMPTY - ERROR',
	...
	public function AddToCart(){
	    ...
	    foreach ($WholeMarkups as $WholeMarkup) {
			...
			if($WholeMarkupPer == 'Person'){
				$WholeMarkupFeeAmount = $WholeMarkupFeeAmount*$SAdults;
				if($SChildren > 0){
					$WholeMarkupFeeAmount = $WholeMarkupFeeAmount*$SChildren;
				}
			}
			...
			if($MrkupFeeHStatus == 'Y'){
				($WholeMarkup[0] == 'Addition') ? $HiddenFeeAmount += $MarkupFeeAmount : $HiddenFeeAmount -= $MarkupFeeAmount;
			}
			else{
				($WholeMarkup[0] == 'Addition') ? $VisibleFeeType = '+' : $VisibleFeeType = '-';
				$VisibleMarkupsArray[] = array($MrkupFeeDesc,$VisibleFeeType,$MarkupFeeAmount,$WholeMarkupPer);
			}
	    }
		...
		if(isset($AirPriceRes->Body->AirPriceRsp->AirPriceResult)){
	    	...
	    	$Taxes = $this->account_model->PercentageToAmount($Taxes,$myMarkup) + $TotalMarkupFeeAmount;
  ...
  public function fareRule(){
		...
		$fare_message = <<<EOM
			<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/">
				<soapenv:Header/>
				<soapenv:Body>
					<ns2:AirFareRulesReq xmlns="http://www.travelport.com/schema/common_v33_0" xmlns:ns2="http://www.travelport.com/schema/air_v33_0" FareRuleType="long" TargetBranch="P7028803" AuthorizedBy="Test">
