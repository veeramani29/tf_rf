<?php
   ...
   // echo'<pre>'; print_r($flights_data);die; 
    ...
    foreach($flights as $flight){
		...
		//Onward Total Time Without Waiting
		$Tot_onward_time =$flight->Onward_TravelTime;
		$exp_OTime=explode("T",$Tot_onward_time);
		$time_Os=str_replace("P","",$exp_OTime[0]);
		$exp_OM=explode("H",$exp_OTime[1]);
		$time_Ostring=substr($time_Os, 0, 1);
		$fnl_Otime_string=explode("M",((24*$time_Ostring)+$exp_OM[0])."H"." ".$exp_OM[1]);
		$Tot_Otraveltime=$fnl_Otime_string[0]."M";
		
		
		//Return Total Time Without Waiting
		$Tot_return_time =$flight->Return_TravelTime;
		$exp_RTime=explode("T",$Tot_return_time);
		$time_Rs=str_replace("P","",$exp_RTime[0]);
		$exp_RM=explode("H",$exp_RTime[1]);
		$time_Rstring=substr($time_Rs, 0, 1);
		$fnl_Rtime_string=explode("M",((24*$time_Rstring)+$exp_RM[0])."H"." ".$exp_RM[1]);
		$Tot_Rtraveltime=$fnl_Rtime_string[0]."M";
    	...
    	$onward_dur = $this->flight_model->get_duration($onward_DepartureDateTime,$onward_ArrivalDateTime);    
    	$return_dur = $this->flight_model->get_duration($return_DepartureDateTime,$return_ArrivalDateTime); 
  ...
  <div class="repflight roundts">
  	<div class="tablpik">
  		...
  		<div class="col-md-8 col-xs-8 nopad fulat500">
  			<div class="col-md-5 col-xs-5">
  				<span class="radiobtnnill rittextalign"><?php echo $segment->Origin;?> <strong><?php echo date('H:i', $DepartureDateTime);?></strong></span>
  			...
  			<div class="col-md-5 col-xs-5">
  				<span class="radiobtnnill"><?php echo $segment->Destination;?> <strong><?php echo date('H:i', $ArrivalDateTime);?></strong></span>
