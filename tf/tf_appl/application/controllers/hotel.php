...
class Hotel extends CI_Controller {
    ...
    public function search() {
        ...
        $data['city_code'] = $_GET['cityId'];
        $data['check_in'] = $check_in;
        $data['check_out'] = $check_out;
       /* $data['rooms'] = $_GET['rooms'];
        $data['adult'] = $_GET['adult'];*/

        for($i=1;$i<=$_GET['total_rooms'];$i++)
        {
            $reqdata['request']['rooms'][$i]=array();
            $reqdata['request']['rooms'][$i]['adult']=$this->input->get('txt'.$i.'plus');
            $reqdata['request']['rooms'][$i]['children']=$this->input->get('txt'.$i.'plusChild');
            for($j=1;$j<=$this->input->get('txt'.$i.'plusChild');$j++){
                $reqdata['request']['rooms'][$i]['childrenage'.$j]=$this->input->get('txt'.$i.'plusChildage'.$j);
            }
        }
        ...
        $_SESSION['hotel_adult'] = $data['total_adults'];

        $_SESSION['travel_port'] = array();
        
        $explode = explode('  - ', $this->input->get('city'));
        $city_code = $_GET['cityId'];
        $check_in = $this->input->get('hotel_checkin');
        $check_out = $this->input->get('hotel_checkout');
        $rooms = $this->input->get('rooms');
        $adult = $this->input->get('total_adults');

        if (!empty($city_code) && !empty($check_in) && !empty($check_out)) 
        {
           $data['total_rooms']=$reqdata['request']['total_rooms'] = $_GET['total_rooms'];
            $data['city_code']=$reqdata['request']['city_code'] = $city_code;
			$cname=explode(", ",$explode[0]);
			$data['city_name']=$reqdata['request']['city_name'] = $cname[0];
			$data['country_name']=$reqdata['request']['country_name'] = $cname[1];
            $data['check_in'] =$reqdata['request']['check_in']= $check_in;
            $data['check_out'] =$reqdata['request']['check_out']= $check_out;
            $data['rooms'] = $rooms;
            $data['total_adults'] = $adult;
            ...
            ///$resdata= get_hotel_list($data);     

            $this->load->view('hotel/search', $reqdata);
        } 
        else {
            redirect('');
        }
    ...
    public function hotel_details() {
        if (isset($_POST['hotelid']) && isset($_POST['city_code']) && isset($_POST['type']) && $_POST['city_code'] != '' && $_POST['hotelid'] != '' && $_POST['type'] != '') {
            //print_r(json_decode($_POST['roomslist']));
            $data['request']['sid'] = $_POST['sid'];
            $data['request']['hotelid'] = $_POST['hotelid'];
            $data['request']['city_code'] = $_POST['city_code'];
            $data['request']['roomslist'] = $_POST['roomslist'];
			$data['request']['minprice'] = $_POST['minprice'];
			if(isset($_GET['room_count'])){
			 $data['request']['room_count'] = $_POST['room_count'];
			}else{
				$data['request']['room_count'] =1;
			}
			
            $data['rooms'] = $_POST['rooms'];
            //echo '<pre/>';
            // print_r($data);exit;
            $data['request']['checkdate'] = 'notok';
            $data['request']['checkin'] = '';
            $data['request']['checkout'] = '';
            if (isset($_GET['checkin']) && isset($_POST['checkout']) && $_POST['checkin'] != '' && $_POST['checkout'] != '') {
                $data['request']['checkdate'] = 'ok';
                $data['request']['checkin'] = $_POST['checkin'];
                $data['request']['checkout'] = $_POST['checkout'];
            }
            $data['request']['adult'] = 1;
            if (isset($_POST['adult']) && $_POST['adult'] != '') {
                $data['request']['adult'] = $_POST['adult'];
            }
            $type = $_POST['type'];
    ...
    public function hotel_search() {
       ...
       $data3['nextlink_set'] =$_POST['nextlink'];
        ...
        $xml = preg_replace("/(<\/?)(\w+):([^>]*>)/", "$1$2$3", $hotel_search_results);
        $xml = simplexml_load_string($xml);
        $json = json_encode($xml);
        $xml_to_array = json_decode($json,true);   

		}
       

		if(isset($xml_to_array['sBody']))
		{
			  $xml_log = array(
          'Api' => 'UAPI',
		...
		$data3['city_code']=$data['city_code'] = $_POST['city_code'];
        $data3['check_in']=$data['check_in']  = $_POST['check_in'];
        $data3['check_out']=$data['check_out']  = $_POST['check_out'];
        $data3['rooms'] =$data['rooms'] = $_POST['rooms'];
        $data3['adult']=$data['adult']  = $_POST['total_adults'];
	   ...
	   $data3['nextlink']=	'';
        ...
        if(isset($xml_to_array['sBody']['SearchResponse']['SearchResult']))
        {
                if (!empty($xml_to_array['sBody']['SearchResponse']['SearchResult']['Hotels']['Hotel'])) {
                    $hotel_search_nodes = $xml_to_array['sBody']['SearchResponse']['SearchResult']['Hotels']['Hotel'];
                    ...
                    if (!empty($hotel_search_nodes)) {
                        for ($k = 0; $k < count($hotel_search_nodes); $k++) {
                           /* if (!empty($hotel_search_nodes[$k]['@attributes']['Code']) && $hotel_search_nodes[$k]['@attributes']['Code'] != 5000) {
                                    ...
                                    $data['Code'] = $hotel_search_nodes[$k]['@attributes']['Code'];
                                    
                                    $data['Name'] = $hotel_search_nodes[$k]['@attributes']['Name'];
                                    $data['MinPrice'] = $hotel_search_nodes[$k]['@attributes']['MinPrice'];
                                    $data['photo'] = $hotel_search_nodes[$k]['@attributes']['Photo'];
                                    $data['address'] = $hotel_search_nodes[$k]['@attributes']['Address'];
                                    $data['lat'] = $hotel_search_nodes[$k]['@attributes']['Lat'];
                                    $data['lon'] = $hotel_search_nodes[$k]['@attributes']['Lon'];
                                    $data['roomslist'] = json_encode($hotel_search_nodes[$k]['Options']);
...
function get_crs_rooms($hotelid, $city_code, $checkin, $checkout, $adult,$roomcount) {
    ...
    function get_hotel_details() {
        ...
        $data['hotelid'] = $_POST['hotelid'];
		 $data['city_code'] = $_POST['city_code'];
		  $data['roomcount'] =  $_POST['city_code'];
		 $data['locationcode'] = '';
        ...
        $hotel_details_results = $returndata['response'];
        $xml_to_array_details = array();
        if($hotel_details_results!='')
        {
        //$xml_to_array = $this->xml_to_array->XmlToArray($hotel_search_results);
        $xml = preg_replace("/(<\/?)(\w+):([^>]*>)/", "$1$2$3", $hotel_details_results);
        $xml = simplexml_load_string($xml);
        $json = json_encode($xml);
        $xml_to_array_details = json_decode($json,true);   

        }

     //print_r($xml_to_array_details); exit;
		$HotelProperty='';
		if(isset($xml_to_array_details['sBody']['HotelDetailResponse']['HotelDetailResult']['HotelDetail']))
		{
        $HotelProperty = $xml_to_array_details['sBody']['HotelDetailResponse']['HotelDetailResult']['HotelDetail'];
		}
		  $HotelDetailItem ='';
        ...
        if (!empty($HotelProperty)) {
            $data['hotel_location'] = $HotelProperty['HotelAddress'];
            $data['hotel_name'] = $HotelProperty['HotelName'];
            $data['hotel_chain'] = $HotelProperty['HotelChainId'];
            $data['hotel_code'] = $HotelProperty['HotelId'];
            $data['hotel_description'] = $HotelProperty['HotelDesc'];
            $data['hotel_area'] = $HotelProperty['Area'];
            $data['hotel_room'] = $HotelProperty['Room'];
            $data['hotel_photos'] = $HotelProperty['HotelPhotos'];
            /*$data['hotel_rating'] = '';
    ...
    //FUNCTIONS BY GANPAT FOR LEONARDO TRAVEL API INTEGRATION    

    //Function for make book request
    public function makebook()
    {
        //This is for ideal integration  
       
       //  $dt = array('amount' => base64_encode(json_encode('10.00')),'entranceCode'=>base64_encode(json_encode('test123')));
       //  return $this->load->ext_view('third_party/ideal/pay', 'requestTransaction', $dt);        
       // exit;
        
        if (preg_match_all('/#(.*?)#/',$_POST['makebook_token'] , $display)) {      
            $data1['room_ids'] = base64_encode(json_encode($display[1]));
        }        
        $data1['rooms'] = $_POST['rooms'];
        $returndata = makebook_request($_POST['makebook_token']);
       
        //Save xml logs 
                $resp = $returndata['response'];
                $resp_xml_arr = array();
                if($resp!='')
                {
                
                $resp_xml = preg_replace("/(<\/?)(\w+):([^>]*>)/", "$1$2$3", $resp);
                $resp_xml = simplexml_load_string($resp_xml);
                $resp_json = json_encode($resp_xml);
                $resp_xml_arr = json_decode($resp_json,true);   

                }               

                if(isset($resp_xml_arr['sBody']))
                {
                      $xml_log = array(
                  'Api' => 'UAPI',
                  'XML_Type' => 'Makebook request ',
                  'XML_Request' => $returndata['request'],
                  'XML_Response' => $returndata['response'],
                  'Ip_address' => $this->input->ip_address(),
                  'XML_Time' => date('Y-m-d H:i:s')
        ...
        $returndata = $this->xml_to_array->XmlToArray($returndata['response']);
        //get hotel cancelation policy
               
        if($returndata['s:Body']['MakeBookResponse']['MakeBookResult']['Status']['MakeBook'] == true)
            {                
                //get hotel details to show on confirmation page
                $hotel_details = get_hotel_details_results(array('hotelid' =>$returndata['s:Body']['MakeBookResponse']['MakeBookResult']['HotelId'] ));                
                
               //for geting hotel detials 
                $hotel_details_results = $hotel_details['response'];
                $xml_to_array_details = array();
                if($hotel_details_results!='')
                {                
                    $xml = preg_replace("/(<\/?)(\w+):([^>]*>)/", "$1$2$3", $hotel_details_results);
                ...
                $data1['hotel_info'] = $this->load->view('hotel/hotel_details_info', $data, true);
                $data1['hotel_images'] = $this->load->view('hotel/hotel_image_slider', $data, true);
              //end getting hotel detials

               
            }
        $data1['makebook_result'] = $returndata['s:Body']['MakeBookResponse']['MakeBookResult'];
        
        $this->load->view('hotel/hotel_book', $data1);
    }

    public function confirmbooking()
    {
        $res = get_confirmation($_POST);
        //Save xml logs 
        $resp = $res['response'];
        $resp_xml_arr = array();
        if($resp!='')
        ...
        if(isset($resp_xml_arr['sBody']))
        {
            $xml_log = array(
                'Api' => 'UAPI',
                'XML_Type' => 'Confirm booking request ',
                'XML_Request' => $res['request'],
                'XML_Response' => $res['response'],
                'Ip_address' => $this->input->ip_address(),
                'XML_Time' => date('Y-m-d H:i:s')
                );
            $this->xml_model->insert_xml_log($xml_log);
