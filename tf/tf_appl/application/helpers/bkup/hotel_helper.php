...
foreach($data['rooms'] as $room){
  $room="<SearchRoom>";
  $room.="<Adult>".$room['adult']."</Adult>";
  for($i=1;$i<=$room['children'];$i++){
    $room.="<Child".$i."Age>".$room['childrenage'.$i]."</Child".$i."Age>";
  }
  $room.="</SearchRoom>";
}
...
function get_hotel_details($data){
...
function curl($xml_request) {
    ...
    $soapAction = 'http://tempuri.org/ILeon/Search';
    ...
    curl_setopt($ch, CURLOPT_URL, 'http://Leon.leonardotravel.com/Leon.svc/wsdl');
