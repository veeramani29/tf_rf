<?php
    ...
    <div id="ticketid0123<?php echo $i;?>" class="maderow flight" data-class="<?php echo ($flight->CabinClass!=null)?$flight->CabinClass:'';?>" data-airline="<?php echo $this->flight_model->get_airline_name($first_seg->Carrier);?>" data-arrive="<?php echo $ArrivalDateTime;?>" data-duration="<?php echo $jms;?>" data-stops="<?php echo count($flight->segments)-1;?>" data-depature="<?php echo $depminutes;?>" data-airlinecode="<?php echo $first_seg->Carrier;?>" data-price="<?php echo $this->account_model->currency_convertor($flight->TotalPrice+$TotalMarkupFeeAmount);?>">
      <div class="col-md-12 nopad">
              ...
              <div class="onwyrow">
                ...
                <div class="col-md-3 col-xs-3 nopad fulat500">
                  ...
                  <div class="fatfi">
                    ...
                    <span class="norto text-primary"><?php echo (($Stops[$i]>1)?$Stops[$i]." ".lang('STOP'):(($Stops[$i]==0)?lang('NONSTOP'):$Stops[$i]." ".lang('STOP')));?></span>

                    <span class="norto text-primary"><?php echo $first_seg->CabinClass;?></span>
                  </div>
                </div>
              </div>
              <div class="clear"></div>
              <div class="pricefilt foronmob">
                <?php $bookingCounts = $flight->segments[0]->BookingCounts;
                ...
                echo "<font color='red'>".(($tickets > 9)?'9':$tickets)." Tickets available </font>";
                ?>
                ...
                <form  name="flightbook<?php echo $i;?>" id="F<?php echo str_replace('.', '', ($flight->TotalPrice+$TotalMarkupFeeAmount)).$i;?>" action="<?php echo WEB_URL;?>">
                  ...
                  <input class="booking selectbtn FlightbookNow" type="button" name="button" data-attr="F<?php echo str_replace('.', '', ($flight->TotalPrice+$TotalMarkupFeeAmount)).$i;?>" value="<?=lang('SELECT');?>" />
                </form>
                <!-- //FlightbookNow -->
              </div>
              <div class="clear"></div>
              <div class="toglerow">
                <div class="clearfix">
                  ...
                  <div class="pull-right text-right">
                   ...
                   <button class="lightbtn" type="button" data-toggle="collapse" data-target="#collapse_ref<?php echo $i;?>"><img src="<?php echo ASSETS;?>images/site/arrow_open.png" class="open_close_arrow"><?php echo $flight->FareType;?>
                    <?php }else{?>
                    <p class="nonrefund_nonexpand"><?php echo $flight->FareType;?></p>
                    <?php }?>
        ...
        <div class="collapse frowexpand refund_expand" id="collapse_ref<?php echo $i;?>">
          <div class="col-md-12 nopad tablshow">
           <div class="onwFltTitle"><i class="fa fa-arrow-right" aria-hidden="true"></i> Refundable Information</span></div>
           <p> CancelPenalty <?php if($flight->CancelPenalty!='') { ?><b>(<?php echo $flight->CancelPenalty?>)</b> <?php }else{ echo "Please click Detailed fare Conditions and Information under ChangePenalty or CancelPenalty";}  ?></p>
           <p> ChangePenalty <?php if($flight->ChangePenalty!='') { ?> <b>(<?php echo $flight->ChangePenalty?>)</b> <?php }else{ echo "Please click Detailed fare Conditions and Information under ChangePenalty or CancelPenalty";}  ?></p>
         </div>
       </div>
       ...
       <div  class="collapse frowexpand" id="collapse_bag<?php echo $i;?>">
        <div class="col-md-12 nopad tablshow">
         ...
         foreach($flight->segments as $key => $segment){
          echo "<table style='width: 90%;margin: 0 15px;'><tr><td><img src='".ASSETS."images/airline_logo/".$segment->Carrier.".gif'  alt='Carrier'></td>";
          echo "<td style='width: 60%;'><h5 class='f_color' style='font-weight: 600;'>Cabin : 7 Kilograms/Person</h5></td></tr>";
          echo "<tr><th><h5 class=''>".$segment->Origin." → ".$segment->Destination."</h5></th>";
          if($segment->BaggageAllowance > 0)
            echo "<td style='width: 60%;'><h5 class='f_color' style='font-weight: 600;'>Check-in : ".$segment->BaggageAllowance."/Person</h5></td></tr></table>";
          elseif(strrev($segment->BaggageAllowance) > 0)
            ...
            echo "<td style='width: 60%;'><h5 class='f_color' style='font-weight: 600;'>No check-in baggage allowance</h5></td></tr></table>";
        }
        ?>
      </div>
    </div>
...
for(var i=0;i<totaldiv;i++){
 $('div.flights div.flight[data-airlinecode="'+ el.value +'"]').filter('div#flights div#ticketid0123'+i).show();
}
});*/
  } else {
...
$filters1.on('ifChanged', function(event){
  ...
  if ($selectedFilters1.length > 0) {
   ...
   /* $selectedFilters1.each(function (i, el) {
      var totaldiv=$('#table_count').val();;
      for(var i=0;i<totaldiv;i++){
        $('div.flights div.flight[data-stops="'+ el.value +'"]').filter('div#flights div#ticketid0123'+i).show();
      }
    });*/
  } else {
